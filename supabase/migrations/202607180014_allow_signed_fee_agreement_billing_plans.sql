-- Allow operational billing to begin after signing and remain available after document closure.
-- Legacy active Fee Agreements remain eligible during the lifecycle transition period.
...
begin
  ...
  if v_agreement.status not in ('signed', 'completed', 'active') then
    raise exception 'Billing plans require a signed, completed, or legacy active fee agreement';
  end if;
...
begin
  ...
  if v_plan.status = 'draft' and v_next_status = 'active' then
    ...
    if v_agreement.id is null or v_agreement.status not in ('signed', 'completed', 'active') then
      raise exception 'Billing plan requires a signed, completed, or legacy active fee agreement';
    end if;
...
begin
  ...
  if v_agreement.status not in ('signed', 'completed', 'active') then
    raise exception 'A default billing plan requires a signed, completed, or legacy active fee agreement';
  end if;
