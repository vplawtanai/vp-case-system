-- Avoid creating Fee Agreement versions when a draft save has no substantive changes.
...
begin
  ...
  perform public.record_finance_fee_agreement_version(
    v_agreement.id,
    case when v_agreement.status = 'under_review' then 'under_review_metadata_saved' else 'draft_metadata_saved' end,
    null,
    jsonb_build_object('fields', jsonb_build_array('title', 'effective_date', 'expiry_date', 'billing_method'))
  );
...
begin
  if not public.current_user_can_manage_finance_quotations() then
    raise exception 'Not allowed to save fee agreement legal terms';
  end if;
  ...
  if v_agreement.legal_terms_json is not distinct from v_legal_terms
  ...
  update public.finance_fee_agreements
  set legal_terms_json = v_legal_terms,
...
revoke all on function public.save_finance_fee_agreement_draft_metadata(uuid, text, date, date, text) from public, anon;
