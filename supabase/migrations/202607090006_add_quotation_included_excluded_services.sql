alter table public.finance_quotations
  add column if not exists included_services text null,
  add column if not exists excluded_services text null;
...
revoke all on function public.save_finance_quotation_draft_impl(
  uuid,
  uuid,
  bigint,
  uuid,
...
set search_path = public
as $$
  select public.save_finance_quotation_draft_impl(
    p_quotation_id,
    p_client_id,
    p_case_id,
...
grant execute on function public.save_finance_quotation_draft(
  uuid,
  uuid,
  bigint,
  uuid,
