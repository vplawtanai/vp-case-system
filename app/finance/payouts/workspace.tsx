...
import { ArrowLeft, Banknote, Calculator, Check, Clock, FileText, Save, Send, UserRound, Wallet } from "lucide-react";
...
import { mask, payoutBlock, payoutError, payoutHref, payoutMath, type Payee, type Workspace } from "./shared";
...
export function PayoutWorkspace({ id, payeeId, fixture }: { id: string; payeeId: string; fixture?: Workspace }) {
 ...
 const math = payoutMath(picked, rates), account = frozen?.account || data?.accounts.find(a => locationKey(a) === accountKey);
 const values = readonly && p ? { gross: p.gross_amount, wht: p.wht_amount, net: p.net_amount } : math;
 const block = payoutBlock(payee, account, values.wht, paidOn);
 const previewReady = readonly || math.valid;
 const payeeReadiness = !payee?.is_active ? "inactive" : account?.kind === "cash" && previewReady && !values.wht ? "ready" : !payee.destination && account?.kind !== "cash" ? "bankMissing" : !payee.tax_id && (!previewReady || values.wht > 0) ? "taxMissing" : "ready";
 const money = (v: number | null | undefined) => v == null ? t("payout.unknown") : `${v.toLocaleString(locale, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} THB`;
 ...
 return <PageShell className={css.page}>
  ...
  <ol className={css.steps}>{["recipient", "reviewStep", "review", "done"].map((s, i) => <li key={s} aria-current={p?.status === "confirmed" ? i === 3 ? "step" : undefined : i === (p ? 2 : 0) ? "step" : undefined}><span>{i + 1}</span>{t(`payout.${s}`)}</li>)}</ol>
   ...
   <div className={css.layout}><div className={css.main}>
    <section className={css.section}><h2><UserRound />1. {t("payout.recipient")}</h2><div className={css.recipient}>
     ...
     {payee ? <div className={css.payee}><strong>{payee.legal_name}</strong><span>{t(`payout.${payee.kind}`)} · {t(`payout.${payee.entity_type}`)}</span><span>{t("payout.taxId")}: {mask(payee.tax_id)}</span><span>{payee.destination?.bank_name || t("payout.bank")}: {mask(payee.destination?.account_number)}</span><span className={css.muted}>{t(`payout.${payeeReadiness}`)}</span>{!readonly ? <button className={ui.secondary} disabled={busy} onClick={() => setEditPayee(payee)}>{t("payout.edit")}</button> : null}</div> : null}
    </div></section>
   ...
   </div><aside className={css.side}><h2><Banknote />{t("payout.summary")}</h2>{summary}<dl className={css.summary}><div><dt>{t("payout.before")}</dt><dd>{money(account?.system_balance)}</dd></div><div><dt>{t("payout.after")}</dt><dd>{previewReady ? money(account?.system_balance == null ? null : account.system_balance - values.net) : "-"}</dd></div></dl>
    {!readonly ? <><p className={css.muted}>{t(dirty ? "payout.dirty" : p ? "payout.saved" : "payout.confirmHelp")}</p>{block ? <Callout tone="warning">{t(`payout.${block}`)}</Callout> : null}{account?.system_balance != null && account.system_balance < values.net ? <Callout tone="warning">{t("payout.negative")}</Callout> : null}<div className={css.reviewList}><strong>{t("payout.checkTitle")}</strong><ul>{["checkPayee", "checkRights", "checkTax", "checkAccount"].map(k => <li key={k}>{t(`payout.${k}`)}</li>)}</ul></div><button className={ui.secondary} disabled={busy} onClick={() => void act("save")}><Save size={18} />{t("payout.save")}</button><button className={ui.primary} disabled={busy || !p || dirty || !!block || !math.valid} onClick={() => { setAck(false); setModal("confirm"); }}><Send size={18} />{t("payout.review")}</button>{p ? <div className={css.other}><button className={ui.danger} disabled={busy} onClick={() => { setAck(false); setModal("cancel"); }}>{t("payout.cancel")}</button></div> : null}</> : null}
   </aside></div>
  ...
  <DetailModal open={!!modal} title={t(modal === "cancel" ? "payout.cancel" : "payout.review")} size="edit" onClose={() => { if (!busy) setModal(null); }} closeOnBackdrop={!busy}>
   <div className={css.form}><p>{t(modal === "cancel" ? "payout.cancelHelp" : "payout.confirmHelp")}</p>{modal === "confirm" ? <><strong>{payee?.legal_name}</strong><p>{locationName(account, locale)} → {account?.kind === "bank" && payee?.destination ? `${payee.destination.bank_name} ${mask(payee.destination.account_number)}` : payee?.legal_name}</p>{summary}</> : null}
