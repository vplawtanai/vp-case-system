...
export function PayoutWorkspace({ id, payeeId: entryPayeeId, fixture }: { id: string; payeeId: string | null; fixture?: Workspace }) {
 ...
 async function act(action: "save" | "confirm" | "cancel") {
  ...
  if (action === "cancel" && (!ack || p?.status !== "draft")) return;
 ...
 const summary = <dl className={css.summary}><div><dt>{t("payout.gross")}</dt><dd>{money(values.gross)}</dd></div><div><dt>{t("payout.withheld")}</dt><dd>{previewReady ? money(values.wht) : "-"}</dd></div><div className={css.net}><dt>{t(p?.status === "cancelled" ? "payout.draftNet" : "payout.net")}</dt><dd>{previewReady ? money(values.net) : "-"}</dd></div></dl>;
 ...
 return <PageShell className={css.page}>
  ...
  {p?.status !== "cancelled" ? <ol className={css.steps} aria-label={t("payout.progress")}>{["stepRecipient", "stepTax", "review", "done"].map((s, i) => <li key={s} data-state={review.completed[i] ? "complete" : review.activeStep === i ? "active" : "pending"} aria-current={review.activeStep === i ? "step" : undefined}><span aria-hidden="true">{review.completed[i] ? <Check size={16} /> : i + 1}</span><div>{t(`payout.${s}`)}<small>{t(`payout.${review.completed[i] ? "stepComplete" : review.activeStep === i ? "stepActive" : "stepPending"}`)}</small></div></li>)}</ol> : null}
   ...
   {p ? <div className={css.lifecycle} data-payout-lifecycle={p.status}><div><StatusBadge status={p.status} label={t(p.status === "cancelled" ? "payout.cancelledDraft" : `payout.${p.status}`)} /> <strong>{t("payout.reference")}: {p.id.slice(0, 8).toUpperCase()}</strong></div>{!readonly && p.status === "draft" ? <button className={ui.danger} disabled={busy} onClick={() => { setAck(false); setModal("cancel"); }}><CircleX size={16} />{t("payout.cancel")}</button> : null}</div> : payee && data.history.some(h => h.status === "draft") ? <div className={css.lifecycle}><span>{t("payout.existingDrafts")}</span><div className={css.payeeActions}>{data.history.filter(h => h.status === "draft").map(h => <Link key={h.id} className={ui.secondary} href={payoutHref(payee.id, h.id)}>{t("payout.openDraft")} {h.id.slice(0, 8).toUpperCase()}</Link>)}</div></div> : null}
   {p?.status === "confirmed" ? <Callout tone="info">{t("payout.readOnly")}</Callout> : null}
   {p?.status === "cancelled" ? <Callout tone="success" role="status"><strong>{t("payout.cancelledResult")}</strong><p>{t("payout.cancelRightsRemain")}</p><p>{t("payout.cancelledEvidence")}{p.cancelled_at ? <> · {t("payout.cancelledAt")}: {date(p.cancelled_at, true)}</> : null}</p><Link className={ui.secondary} href="/finance/payables"><ArrowLeft size={16} />{t("payout.backPayables")}</Link></Callout> : null}
   ...
   </div><aside className={css.side}><h2><Banknote />{t("payout.summary")}</h2>{summary}<dl className={css.summary}><div><dt>{t("payout.account")}</dt><dd>{account ? locationName(account, locale) : t("payout.chooseAccount")}</dd></div>{p?.status !== "cancelled" ? <><div><dt>{t("payout.before")}</dt><dd>{balance}</dd></div><div><dt>{t("payout.after")}</dt><dd>{balanceAfter}</dd></div></> : null}</dl>
    {!readonly ? <>
     ...
     <button className={ui.primary} disabled={disabledReasons.length > 0} aria-describedby={disabledReasons.length ? "payout-review-blockers" : undefined} onClick={() => { setAck(false); setPaidAck(false); setModal("confirm"); }}><Send size={18} />{t("payout.review")}</button>
    </> : null}
   </aside></div>
  ...
  <DetailModal open={!!modal} title={t(modal === "cancel" ? "payout.cancelTitle" : "payout.review")} size="edit" onClose={() => { if (!busy) setModal(null); }} closeOnBackdrop={!busy}>
   <div className={css.form}><p>{t(modal === "cancel" ? "payout.cancelIntro" : "payout.confirmHelp")}</p>{modal === "cancel" ? <>
    ...
    <label className={css.check}><input type="checkbox" checked={ack} disabled={busy} onChange={e => setAck(e.target.checked)} /><span>{t(modal === "cancel" ? "payout.cancelAck" : "payout.ackReviewed")}</span></label>
    {modal === "confirm" ? <>
    ...
    {modal === "cancel" ? <button className={ui.secondary} disabled={busy} onClick={() => setModal(null)}>{t("payout.cancelBack")}</button> : null}
    <button className={modal === "cancel" ? ui.danger : ui.primary} disabled={busy || !ack || (modal === "confirm" && !paidAck)} aria-describedby={modal === "confirm" && (!ack || !paidAck) ? "payout-confirm-ack-required" : undefined} onClick={() => void act(modal === "cancel" ? "cancel" : "confirm")}><Check size={18} />{t(modal === "cancel" ? "payout.cancelConfirm" : "payout.confirm")}</button>
   </div>
  </DetailModal>
 </PageShell>;
}
