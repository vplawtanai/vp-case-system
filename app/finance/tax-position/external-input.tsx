...
import { externalVatStatus, type ExternalVat } from "./period-data";
...
export function ExternalInputForm({ row, onClose, onSaved }: { row?: ExternalVat; onClose: () => void; onSaved: () => void }) {
 ...
 const [status,setStatus]=useState<string>(row ? externalVatStatus(row) : "eligible"),[reason,setReason]=useState(""),[ack,setAck]=useState(false),[busy,setBusy]=useState(false),[failed,setFailed]=useState(false);
 ...
 return <DetailModal variant="review" open title={tr("external")} size="edit" onClose={()=>{if(!busy)onClose();}} closeOnBackdrop={false}>
  <form className={css.form} onSubmit={submit}><p>{tr("funding")}</p><Callout tone="info">{tr("noCash")} {!row?tr("immediateInput"):null}</Callout>
