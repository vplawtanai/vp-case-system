...
import { filingCoverage, filingInput, filingMonthlyFacts, type FilingPool } from "./shared";
...
export function VatFilingReview({ pool, monthlyFacts, outgoingWht }: {
 ...
 const facts = filingMonthlyFacts(pool, monthlyFacts), input = filingInput(pool), coverage = filingCoverage(pool);
 const outputKnown = facts?.outputVat != null;
 ...
 // Version 2 uses the server pool/frozen snapshot, never a live replacement for its VAT.
 return <div className={styles.vatReview}>
  <ReadOnlyGrid className={styles.vatFacts} items={[
   { key: "output", label: tr("output"), value: money(facts?.outputVat), emphasis: true },
   { key: "input", label: tr("input"), value: input.input_vat_complete ? money(input.input_vat) : tr("incomplete") },
  ...
  <Callout tone={pool.ready ? "info" : "warning"} role="status">
   <div className={styles.vatReadiness}>
    ...
    <div><strong>{tr(pool.ready ? "evidenceReadyForReview" : "vatNotReady")}</strong>
     {!pool.ready ? <p>{tr(!input.input_vat_complete ? "vatInputBlockReason" : "notReady")}</p> : null}
    </div>
   </div>
  </Callout>
  <dl className={styles.vatCoverage}><div><dt>{tr("draftCoverage")}</dt><dd>{tr(coverage.source_count === 0 ? "notCollected" : pool.ready ? "evidenceReadyForReview" : "incomplete")}</dd></div></dl>
  <section aria-labelledby="vat-review-checks"><h3 id="vat-review-checks">{tr("vatReviewChecks")}</h3><ul className={styles.checklist}>
   {!input.input_vat_complete ? <li><TriangleAlert size={17} aria-hidden="true" /><span>{tr("inputReview")}</span></li> : null}
