...
export type BillableChargeWorkflowHandle = { isDirty: () => boolean; isBusy: () => boolean };

type Props = {
  ref?: Ref<BillableChargeWorkflowHandle>;
...
export default function BillableChargeCreateWorkflow({
  ...
  useImperativeHandle(ref, () => ({ isDirty: () => dirty, isBusy: () => actionLockRef.current }));
  ...
  const saveAction = <div className={styles.saveRow}><span className={dirty ? styles.unsavedState : styles.savedState}>{dirty ? t("finance.charge.ui.unsaved") : charge ? t("finance.charge.ui.draftSaved") : t("finance.charge.ui.notCreated")}</span><button className={actionContainer && (!charge || dirty) ? styles.primaryButton : styles.secondaryButton} type="button" disabled={saving || !dirty || !canManage} onClick={() => void saveDraft()}>{saving ? t("finance.invoice.ui.saving") : t("finance.charge.ui.saveDraft")}</button></div>;
  const readyAction = charge ? <button className={styles.primaryButton} type="button" disabled={saving || dirty || !canApprove} onClick={() => void markReady()}>{t("finance.invoice.ui.audit.ready")}</button> : null;
