...
export function TaxInvoiceDocument({ row, logoUrl }: { row: TaxInvoice; logoUrl: string }) {
  ...
  return <LegalDocumentLayout className={`${styles.paper} ${theme.taxInvoice}`} languageCode="th">
    ...
    <table className={styles.documentTable}><thead><tr><th scope="col">รายการ</th><th scope="col">ก่อน VAT</th><th scope="col">VAT</th><th scope="col">รวม THB</th></tr></thead><tbody><tr><td>{d.description}</td><td>{taxMoney(d.beforeVat)}</td><td>{taxMoney(d.vat)}</td><td>{taxMoney(d.gross)}</td></tr></tbody></table>
    <dl className={styles.documentTotals}><div><dt>มูลค่าบริการก่อน VAT</dt><dd>{taxMoney(d.beforeVat)}</dd></div><div><dt>ภาษีมูลค่าเพิ่ม {d.vatRate}%</dt><dd>{taxMoney(d.vat)}</dd></div><div><dt>ยอดรวม THB</dt><dd>{taxMoney(d.gross)}</dd></div></dl>
