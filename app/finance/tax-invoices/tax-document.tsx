...
export function TaxInvoiceDocument({ row, logoUrl, documentNotice, documentNoOverride }: { row: TaxInvoice; logoUrl: string; documentNotice?: React.ReactNode; documentNoOverride?: string }) {
  ...
  return <LegalDocumentLayout className={`${styles.paper} ${theme.taxInvoice}`} languageCode="th">
    ...
    </div><dl className={styles.documentFacts}><div><dt>วันที่ออกใบกำกับภาษี</dt><dd>{taxDate(d.issueDate)}</dd></div><div><dt>วันที่เกิดภาษี</dt><dd>{taxDate(d.taxPointDate)}</dd></div><div><dt>{d.sourceType ? "อ้างอิงรายการรับเงิน" : "ใบแจ้งหนี้อ้างอิง"}</dt><dd>{d.invoiceNumber}</dd></div>{d.receiptNumber ? <div><dt>ใบเสร็จอ้างอิง</dt><dd>{d.receiptNumber}</dd></div> : null}</dl></section>
