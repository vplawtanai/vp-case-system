...
function Workspace({ id, permissions }: { id: string; permissions: UserPermissions }) {
  ...
  return <>
    <header className={`${styles.header} ${styles.noPrint}`}><div><Link className={styles.button} href="/finance/tax-invoices">{t("finance.taxInvoice.ui.back")}</Link><h1>{state.row?.tax_invoice_no || t("finance.taxInvoice.ui.draftTitle", { reference: id.slice(0, 8).toUpperCase() })}</h1>{state.row ? <FinanceStatusBadge status={state.row.status} label={taxStatusLabel(state.row.status, locale)}/> : null}</div></header>
