...
function Workspace({ id, permissions }: { id: string; permissions: UserPermissions }) {
  ...
  return <>
    ...
    {state.loading ? <p role="status">กำลังโหลด...</p> : state.error ? <p role="alert" className={styles.error}>{state.error} <button className={styles.button} onClick={() => void state.reload()}>โหลดใหม่</button></p> : state.row ? <Editor key={state.row.updated_at} row={state.row} permissions={permissions} logoUrl={state.logoUrl} blockers={state.blockers} reload={state.reload} /> : null}
  </>;
}
function Editor({ row, permissions, logoUrl, blockers, reload }: { row: TaxInvoice; permissions: UserPermissions; logoUrl: string; blockers: string[]; reload: () => Promise<void> }) {
  ...
  const draft = row.status === "draft", editable = draft && permissions.canManageFinanceTaxInvoices;
  ...
  const ready = taxIssueReady({ status: row.status, canIssue: permissions.canIssueFinanceTaxInvoices, busy, dirty, blockerCount: blockers.length,
  ...
  async function run(action: "save" | "refresh" | "issue" | "cancel") {
    ...
    try {
      const result = action === "save" ? await supabase.rpc("save_finance_tax_invoice_draft", { p_tax_invoice_id: row.id, p_issue_date: issueDate, p_decisions_json: decisions, p_expected_updated_at: row.updated_at })
      ...
      if (result.error || result.data !== row.id) throw result.error || new Error("response");
      await reload();
    } catch (cause) { setError(taxError(cause)); requestAnimationFrame(() => alert.current?.focus()); }
    finally { lock.current = false; setBusy(false); }
  }
  return <>
    <section className={`${styles.section} ${styles.noPrint}`}><div className={styles.actions}><Link className={styles.button} href={`/finance/payments/${row.payment_id}`}>เปิดรายการรับชำระต้นทาง</Link><Link className={styles.button} href={`/finance/invoices/${row.invoice_id}`}>{d.invoiceNumber}</Link><Link className={styles.button} href={`/finance/tax-invoices/${row.id}/preview`}>ดูตัวอย่าง / พิมพ์</Link></div></section>
    <section className={`${styles.section} ${styles.noPrint}`}><h2>ข้อมูลภาษีจากต้นทาง</h2><dl className={styles.facts}><div><dt>ก่อน VAT</dt><dd>{taxMoney(d.beforeVat)} THB</dd></div><div><dt>VAT {d.vatRate}%</dt><dd>{taxMoney(d.vat)} THB</dd></div><div><dt>ยอดรวม</dt><dd>{taxMoney(d.gross)} THB</dd></div><div><dt>เงินที่ได้รับจริง</dt><dd>{taxMoney(d.cash)} THB</dd></div><div><dt>หัก ณ ที่จ่าย</dt><dd>{taxMoney(d.wht)} THB</dd></div></dl></section>
    {draft ? <>
      <fieldset className={`${styles.section} ${styles.noPrint}`} disabled={!editable || busy}><legend>ข้อมูลผู้รับบริการและ VAT</legend><div className={styles.grid}>
        ...
        <label className={styles.field}>ประเภท VAT ของรายการ<select className={styles.input} value={decisions.tax_treatment || ""} onChange={e => change("tax_treatment", e.target.value)}><option value="">ยังไม่ยืนยัน</option><option value="standard_rated">อยู่ในบังคับ VAT ตามอัตราต้นทาง</option><option value="zero_rated">อัตรา 0% พร้อมหลักฐาน</option></select></label>
        {decisions.tax_treatment === "zero_rated" ? <label className={styles.field}>หลักฐานการใช้อัตรา 0%<textarea className={styles.input} rows={2} maxLength={2000} value={decisions.treatment_reason || ""} onChange={e => change("treatment_reason", e.target.value)} /></label> : null}
      </div></fieldset>
      <fieldset className={`${styles.section} ${styles.noPrint}`} disabled={!editable || busy}><legend>จุดเกิดภาษีและวันที่ออก</legend><p>วันที่รับชำระจากต้นทาง: <strong>{taxDate(d.taxPointDate)}</strong></p>
        ...
        <label className={styles.check}><input type="checkbox" checked={decisions.external_coverage_checked === true} onChange={e => change("external_coverage_checked", e.target.checked)} />ตรวจสอบแล้วว่าไม่มีใบกำกับภาษีภายนอกสำหรับยอดนี้ และไม่มีเลข VP-TI ที่ออกหรือจองไว้นอกระบบ</label>
    ...
    <div className={styles.preview}><TaxInvoiceDocument row={row} logoUrl={logoUrl} /></div>
    <div ref={alert} tabIndex={-1} className={styles.noPrint}>{error ? <p className={styles.error} role="alert">{error}</p> : null}</div>
    {draft ? <>
      <section className={`${styles.section} ${styles.noPrint}`}><h2>ยืนยันการออกใบกำกับภาษี</h2>
        ...
        {permissions.canIssueFinanceTaxInvoices ? <>
          ...
          <label className={styles.check}><input type="checkbox" disabled={busy || dirty || !reviewed} checked={ack} onChange={e => setAck(e.target.checked)} />ยืนยันออกเลข VP-TI ถาวรและล็อกหลักฐานเอกสาร ไม่สามารถยกเลิกเอกสารที่ออกแล้วในขั้นตอนนี้</label>
          <button className={styles.primary} disabled={!ready} onClick={() => void run("issue")}>{busy ? "กำลังดำเนินการ..." : "ออกใบกำกับภาษี"}</button>
        </> : <p className={styles.small}>ต้องให้ผู้มีสิทธิ์ออกใบกำกับภาษีตรวจสอบขั้นสุดท้าย</p>}
      </section>
