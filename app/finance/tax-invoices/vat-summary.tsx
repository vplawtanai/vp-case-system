...
import { unknownTaxDecision, unknownVatExplanation, vatTreatmentPresentation, type InvoiceVatLine } from "./vat-treatment";
  ...
  return <div className={styles.vatSummary}>
    ...
    {error ? <p className={styles.warning} role="alert">{error}</p> : lines ? <>
      <table className={styles.vatTable} aria-label="สถานะ VAT ของรายการต้นทาง">
        <thead><tr><th scope="col">รายการ</th><th scope="col">มูลค่าก่อน VAT</th><th scope="col">VAT Treatment</th><th scope="col">VAT</th><th scope="col">สถานะใบกำกับภาษี</th></tr></thead>
        <tbody>{lines.map(line => {
          ...
          return <tr key={`${line.invoiceId}:${line.id}`}>
            ...
            <td data-label="VAT Treatment"><span className={display.workflow === "unresolved" ? styles.vatUnknown : styles.vatKnown}>{display.label}</span></td>
    ...
    {eligibility ? <div className={styles.vatReadiness}>
      ...
      {eligibility.blockers.length ? <ul>{eligibility.blockers.map(code => <li key={code}>{taxError(code)}</li>)}</ul> : null}
    </div> : null}
  </div>;
}
