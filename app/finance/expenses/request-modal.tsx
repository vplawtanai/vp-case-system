  ...
  <DetailModal open size={claim ? "workflow" : "detail"} title={t(claim ? "expenses.newClaimRequest" : purchase ? "expenses.purchaseRequestCreate" : "expenses.companyNewRequest")} onClose={close} closeOnBackdrop={false}
   ...
   <div className={`${css.page} ${claim ? "" : company.create}`}>
    ...
    <section className={`${css.requestSummary} ${claim ? "" : company.createSummary}`} aria-label={t("expenses.requestSummary")}>
     ...
     <dl className={css.requestTotals} aria-live="polite"><div><dt>{t("expenses.itemCount")}</dt><dd>{t("expenses.count", { count: items.length })}</dd></div>{!claim ? <div><dt>{t("expenses.expenseTotal")}</dt><dd>{money(total("gross_amount"))}</dd></div> : null}{reimbursement ? <div><dt>{t(claim ? "expenses.requestedTotal" : "expenses.staffRequestedTotal")}</dt><dd>{money(total("reimbursement_requested"))}</dd></div> : null}</dl>
    </section>
