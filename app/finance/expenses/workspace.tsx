...
import { ExpenseRequestRow } from "./request-view";
import { ClaimRequestReview, ClaimItemReview, claimStatus } from "./claim-review";
...
export function ExpenseWorkspace({ id, claims = false, initialTaxFilter = false, fixture, fixtureLookups }: { id?: string; claims?: boolean; initialTaxFilter?: boolean; fixture?: ExpenseData; fixtureLookups?: ExpenseLookups }) {
 ...
 return <PageShell><div className={css.page}>
  ...
  {loading ? <p role="status">{t("expenses.loading")}</p> : data && access ? <>
   {creating ? canCreate ? (claims || access.company_purchase_request_supported) ? <ExpenseRequestModal claim={claims} access={access} accounts={data.accounts} lookups={lookups} run={run} busy={busy} error={error} onClose={() => router.push(claims ? "/finance/expenses/claims" : "/finance/expenses")} onSaved={id => router.push(`${claims ? "/finance/expenses/claims" : "/finance/expenses"}?request=${id}`)} /> : <ExpenseFactsForm claim={claims} access={access} accounts={data.accounts} lookups={lookups} run={run} busy={busy} /> : <Callout tone="warning">{t("expenses.denied")}</Callout> : record ?
  ...
  {selectedRequest && !editRequestId && data && access ? claims ? <ClaimRequestReview key={selectedRequest.id} request={selectedRequest} access={access} accounts={data.accounts} lookups={lookups} busy={busy} error={error} run={run} onClose={() => { setRequestId(null); setError(""); }} onEdit={() => setEditRequestId(requestId)} /> : <CompanyRequestReview key={selectedRequest.id} request={selectedRequest} access={access} accounts={data.accounts} lookups={lookups} busy={busy} error={error} run={run} onClose={() => { setRequestId(null); setError(""); }} onEdit={() => setEditRequestId(requestId)} /> : null}
 </div></PageShell>;
}
 ...
 return <>
  ...
  <section aria-label={t(viewAll ? "expenses.allClaims" : "expenses.ownClaims")}><div className={css.sectionHead}><h2>{t(viewAll ? "expenses.allClaims" : "expenses.ownClaims")} <span>({filtered.length})</span></h2></div>
   {visible.length ? <table className={`${css.table} ${css.claimTable}`}><thead><tr>{["reference", "requestClaimant", "itemCount", "requested", "approved", "status", "requestSubmittedAt", "action"].map(k => <th key={k}>{t(`expenses.${k}`)}</th>)}</tr></thead><tbody>{visible.map(entry => { if (entry.request) return <ExpenseRequestRow access={access} key={entry.id} request={entry.request} onOpen={() => onRequest(entry.id)} />; const row = entry.expense; return <tr key={row.id}>
    ...
    <td data-label={t("expenses.requestClaimant")}>{row.claimant_name}</td>
    <td data-label={t("expenses.itemCount")}>1<small>{row.description}</small></td>
    ...
    <td data-label={t("expenses.status")}><ExpenseBadge state={claimStatus([row])} /><small className={css.workflowDate}><WorkflowDate value={claimWorkflowTime(row)} /></small></td>
    <td data-label={t("expenses.requestSubmittedAt")}>{date(row.submitted_at)}</td>
...
function ExpenseDetail({ data, row, lookups, run, busy }: { data: ExpenseData; row: Expense; lookups: ExpenseLookups; run: ExpenseRun; busy: boolean }) {
 ...
 if (row.origin === "employee_claim" && row.status !== "draft") return <><ClaimItemReview row={row} access={access} accounts={data.accounts} lookups={lookups} run={run} busy={busy}/><ExpenseActivity row={row} isAdmin={access.is_admin}/></>;
