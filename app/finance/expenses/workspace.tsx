...
export function ExpenseWorkspace({ id, claims = false, initialTaxFilter = false, fixture, fixtureLookups }: { id?: string; claims?: boolean; initialTaxFilter?: boolean; fixture?: ExpenseData; fixtureLookups?: ExpenseLookups }) {
 ...
 return <PageShell><div className={css.page}>
  ...
  {loading ? <p role="status">{t("expenses.loading")}</p> : data && access ? <>
   {creating ? canCreate ? !claims && access.company_purchase_request_supported ? <ExpenseRequestModal claim={false} access={access} accounts={data.accounts} lookups={lookups} run={run} busy={busy} error={error} onClose={() => router.push("/finance/expenses")} onSaved={id => router.push(`/finance/expenses?request=${id}`)} /> : <ExpenseFactsForm claim={claims} access={access} accounts={data.accounts} lookups={lookups} run={run} busy={busy} /> : <Callout tone="warning">{t("expenses.denied")}</Callout> : record ?
