...
export function CompanyExpenseList({ requests, access, onRequest, initialTaxFilter = false }: { requests: ExpenseRequest[]; access: ExpenseAccess; onRequest: (id: string) => void; initialTaxFilter?: boolean }) {
 ...
 return <>
  <div className={css.stats}>{companySummary(requests).map(s => { const Icon = icons[s.lane]; return <article className={css.stat} key={s.lane}><span className={css.icon}><Icon size={24} aria-hidden="true" /></span><div><span>{t(`expenses.${s.lane === "rejected" ? "requestRejected" : s.lane}`)}</span><strong>{t("expenses.companyRequestCount", { count: s.requests })}</strong><small>{t("expenses.count", { count: s.items })}<br />{money(s.amount)}</small></div></article>; })}</div>
  <div className={css.filters}><FieldGroup id="company-search" label={t("expenses.companySearch")}><input type="search" value={search} onChange={e => { setSearch(e.target.value); setPage(0); }} /></FieldGroup><FieldGroup id="company-status" label={t("expenses.status")}><select value={status} onChange={e => { setStatus(e.target.value); setPage(0); }}>{["all", "draft", "review", "unpaid", "paid", "rejected", "accepted", "tax"].map(s => <option key={s} value={s}>{t(`expenses.${s === "rejected" ? "requestRejected" : s === "accepted" ? "requestAccepted" : s}`)}</option>)}</select></FieldGroup><QueueSort id="company-order" value={order} onChange={v => { setOrder(v); setPage(0); }} /></div>
