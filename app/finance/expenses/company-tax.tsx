...
export function CompanyTaxForm({ row, reason, busy, run, onPlan, onCalculation, planning, inputVatOnly = false }: { row: Expense; reason: string; busy: boolean; run: ExpenseRun; planning: boolean; onPlan: (args: Record<string, unknown> | null) => void; onCalculation: (value: CompanyTaxCalculation | null) => void; inputVatOnly?: boolean }) {
 ...
 // Input VAT evidence review preserves the accepted calculation/withholding choices.
 const input = useMemo(() => ({ ...form, schema_version: 2, vat_rate: Number(form.vat_rate), wht_rate: Number(form.wht_rate), ...(inputVatOnly ? { paid_withholding_ack: saved?.paid_withholding_ack === true } : {}), reason: planning ? reason : note }), [form, planning, reason, note, inputVatOnly, saved?.paid_withholding_ack]);
 ...
 const select = (field: keyof typeof form, label: string, choices: [string, string][]) => <FieldGroup id={`company-tax-${field}`} label={t(`expenses.${label}`)}><select disabled={inputVatOnly && (field === "vat_mode" || field === "vat_rate")} value={String(form[field])} onChange={e => set(field, e.target.value)}>{choices.map(([value, label]) => <option key={value} value={value}>{t(inputVatOnly && field === "eligibility" && value === "pending" ? "taxHome.sourceVat" : `expenses.${label}`)}</option>)}</select></FieldGroup>;
 return <form className={inputVatOnly ? css.form : `${css.form} ${company.taxReview}`} data-review-plan="tax" onSubmit={e => { e.preventDefault(); if (!planning && calculation?.ready) void run("review_finance_expense_tax", args); }}>
  <fieldset disabled={busy}><legend>{t("expenses.companyTaxDecision")}</legend><div className={css.formGrid}>
   ...
   {!inputVatOnly ? <>{select("wht_state", "wht", [["", "choose"], ["none", "companyWhtNone"], ["withhold", "companyWhtWithhold"]])}
   {form.wht_state === "withhold" ? select("wht_rate", "whtRate", [["1", "companyRate1"], ["3", "companyRate3"], ["5", "companyRate5"]]) : null}
   {form.wht_state === "withhold" && row.creator_payment_fact === "company_paid" ? <label className={css.check}><input type="checkbox" checked={form.paid_withholding_ack} onChange={e => set("paid_withholding_ack",e.target.checked)} /><span>{t("expenses.companyPaidWhtAck")}</span></label> : null}</> : null}
  ...
  {!inputVatOnly ? <small>{t("expenses.companyMetadataHelp")}</small> : null}
