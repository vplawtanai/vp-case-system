export type ExpenseAccess = { user_id: string; can_claim: boolean; can_manage: boolean; can_tax_review: boolean; can_view_all: boolean; can_view_accounts?: boolean; can_record: boolean; can_confirm: boolean; is_admin: boolean };
...
export type ExpenseSettlement = { id: string; mode: SettlementMode; payee_id: string | null; amount: number; reason: string };
...
export type Expense = { id: string; reference: string; origin: "company_purchase" | "employee_claim" | "legacy_claim"; legacy_claim_id: string | null; status: "draft" | "submitted" | "accepted" | "rejected"; version: number; expense_date: string; description: string; category: string; vendor_name: string | null; supplier_payee_id: string | null; claimant_id: string | null; claimant_name: string | null; client_id: string | null; case_id: number | null; advisory_matter_id: string | null; gross_amount: number; currency: string; personally_paid: boolean; reimbursement_requested: number; vat_awareness: string; wht_awareness: string; note: string; created_by: string; submitted_at: string | null; review_reason: string | null; created_at: string; tax_review: ExpenseTax | null; settlement: ExpenseSettlement | null; obligation: ExpenseObligation | null; payout: ExpensePayout | null; audit: { id: string; event_type: string; actor_name: string; created_at: string; evidence_json: unknown }[] };
export type ExpenseData = { access: ExpenseAccess; accounts: ExpenseAccount[]; rows: Expense[]; record?: Expense; has_next: boolean };
