...
export function ClaimItemReview({ row, access, accounts, lookups, run, busy }: Props & { row: Expense }) {
 ...
 const work = linkedCase ? `${linkedCase.file_no} ${linkedCase.title}` : matter ? `${matter.matter_no} ${matter.title}` : undefined;
 ...
 return <div className={css.review} data-claim-review>
  <section className={css.facts}><div><h3>{row.description || expenseCategoryLabel(row.category,locale)}</h3><p>{t("expenses.claimPaidDate")}: {date(row.expense_date)} · {expenseCategoryLabel(row.category,locale)}</p>{row.vendor_name?<p>{t("expenses.vendor")}: {row.vendor_name}</p>:null}</div><div className={css.amount}><span>{t("expenses.claimPersonalAmount")}</span><strong>{money(row.gross_amount)}</strong></div></section>
