  ...
  {!data || !source ? !error ? <p role="status">{t("common.state.loading")}</p> : null : <>
   ...
   <div className={css.detailGrid} aria-busy={busy}>
    ...
    <section className={`${css.panel} ${css.formula}`}><h2>{w("formula")}</h2>{source.lines.filter(l => l.classification === "professional_fee").map(line => <section className={css.line} key={distributionLineId(line)}>
     ...
     {(!editable || calculated?.errors[distributionLineId(line)]?.length === 0) && choices.find(c => distributionLineId(c) === distributionLineId(line))?.formula_result ? <div className={css.participants}><table className={css.table}><thead><tr><th>{w("recipient")}</th><th>{w("role")}</th><th>{w("percent")}</th><th>{w("amount")}</th></tr></thead><tbody>{choices.find(c => distributionLineId(c) === distributionLineId(line))!.formula_result!.recipients.map(r => <tr key={r.component_no}><td>{r.recipient_kind === "company" ? w("company") : r.recipient_name}</td><td>{compensationRoleLabel(r.role_label, locale)}</td><td>{r.percent ?? (line.professional_pool ? (r.amount / line.professional_pool * 100).toFixed(2) : "—")}{line.professional_pool ? "%" : ""}</td><td className={css.number}>{money(r.amount)}</td></tr>)}</tbody><tfoot><tr><th colSpan={2}>{w("total")}</th><th>{line.professional_pool ? "100%" : "—"}</th><th className={css.number}>{money(choices.find(c => distributionLineId(c) === distributionLineId(line))!.formula_result!.recipients.reduce((n, r) => n + Math.round(r.amount * 100), 0) / 100)}</th></tr></tfoot></table></div> : null}
