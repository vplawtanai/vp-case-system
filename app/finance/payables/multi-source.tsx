...
export function MultiSourcePayables({ canReadRevenue, canReadExpense, isAdmin, fixture }: { canReadRevenue: boolean; canReadExpense: boolean; isAdmin: boolean; fixture?: { revenue: PayableGroup[]; expenses: ExpenseObligation[] } }) {
 ...
 return <PageShell><div className={css.page}><header className={css.heading}><div className={css.title}><span className={css.icon}><FileText size={23} /></span><div><h1>{t("payables.title")}</h1><p>{t("expenses.payablesHelp")}</p></div></div><button type="button" className={ui.secondary} aria-label={t("expenses.refresh")} title={t("expenses.refresh")} disabled={loading} onClick={() => void load()}><RefreshCw size={17} /></button></header>
  ...
  {error ? <Callout tone="negative" role="alert">{t("expenses.failed")}</Callout> : loading ? <p role="status">{t("expenses.loading")}</p> : <>
   {revenue.length ? <section><PayableSourceHeading source="revenue_distribution" /><PayableGroups groups={revenue} isAdmin={isAdmin} /></section> : null}
   {(["employee_reimbursement", "supplier_payable"] as const).map(family => expenses.some(e => e.source_type === family) ? <ExpenseObligationQueue key={family} rows={expenses.filter(e => e.source_type === family)} /> : null)}
...
export function ExpenseObligationQueue({ rows }: { rows: ExpenseObligation[] }) {
 ...
 return <section><PayableSourceHeading source={rows[0].source_type} /><div className={css.queue}><div className={css.queueList}>{rows.map(r => <article key={`${r.source_type}:${r.id}`} className={`${css.queueRow} ${selected?.id === r.id ? css.selected : ""}`}><div className={styles.identity}><RecipientAvatar kind={r.source_type === "employee_reimbursement" ? "person" : "supplier"} /><div><h3 className={styles.queueName}>{r.payee_name}</h3><PayableSourceBadge source={r.source_type} /><p>{r.description}</p></div></div><div><strong>{money(r.gross_amount)}</strong><p><button type="button" className={ui.secondary} aria-pressed={selected?.id === r.id} onClick={() => setSelected(r.id)}>{t("expenses.view")}</button></p></div></article>)}</div>
