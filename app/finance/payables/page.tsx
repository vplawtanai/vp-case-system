...
export function PayablesWorkspace() {
  ...
  return <PageShell>
    ...
    {loading ? <p role="status">{t("common.state.loading")}</p> : failed ? <Callout tone="negative" role="alert">{t("payables.failed")} <button type="button" className={ui.secondary} onClick={() => void load()}>{t("common.actions.retry")}</button></Callout> : data?.groups.length ? <PayableGroups groups={data.groups} /> : <EmptyState>{t("payables.empty")}</EmptyState>}
    <div className={styles.pagination}><button className={ui.secondary} type="button" title={t("finance.receipt.previous")} aria-label={t("finance.receipt.previous")} disabled={loading || offset === 0} onClick={() => page(Math.max(0, offset - 25))}><ArrowLeft size={18} /></button><span>{t("finance.receipt.page", { page: offset / 25 + 1 })}</span><button className={ui.secondary} type="button" title={t("finance.receipt.next")} aria-label={t("finance.receipt.next")} disabled={loading || failed || !data?.has_next} onClick={() => page(offset + 25)}><ArrowRight size={18} /></button></div>
  </PageShell>;
}
