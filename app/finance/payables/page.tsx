...
export function PayableGroups({ groups }: { groups: PayableGroup[] }) {
  ...
  return <div className={styles.groups}>{groups.map(group => <article className={styles.group} key={payableGroupKey(group)} data-recipient={payableGroupKey(group)}>
    <div className={styles.groupHeading}><div><h2>{group.recipient_name}</h2><span className={styles.muted}>{t(group.components[0]?.recipient_type === "payee" ? "payout.external" : "payables.user")}</span>
      <Link className={ui.secondary} href={payoutHref(group.recipient_id)}>{t("payout.title")}<ArrowRight size={16} /></Link>
