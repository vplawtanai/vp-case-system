...
function Detail({ permissions }: { permissions: { canEditFinanceQuotation: boolean } }) {
  ...
  const cancelAcceptedQuotationEngagement = async () => {
    ...
    const reason = window.prompt("ระบุเหตุผลการยกเลิกการว่าจ้างตามใบเสนอราคา");
    if (!reason?.trim()) return;
    if (!window.confirm("ยืนยันการยกเลิกการว่าจ้างตามใบเสนอราคานี้?\n\nรายการจะถูกเก็บไว้เพื่อการตรวจสอบและไม่สามารถดำเนินการต่อได้")) return;
    ...
    try {
      ...
      await load(); setMessage("ยกเลิกการว่าจ้างตามใบเสนอราคาแล้ว");
    } catch (cancelError) {
  ...
  const readinessIssues = (() => {
    ...
    if (dirty) issues.push("บันทึกข้อมูลที่แก้ไขก่อนบันทึกว่าส่งเอกสารให้ลูกค้าแล้ว");
  ...
  const billingPlanDescription = billingPlanNextStepDescription(billingPlan);
  ...
  return <main style={page}><FinanceSubNav activePage="fee-agreements" permissions={permissions as never} /><nav className="fee-agreement-navigation-toolbar" style={navigationToolbar} aria-label="การนำทางและส่งออกเอกสาร"><div className="fee-agreement-navigation-group" style={navigationGroup}><Link className="fee-agreement-navigation-link fee-agreement-navigation-back" style={{ ...navigationLink, ...navigationBackLink }} href="/finance/fee-agreements"><NavigationIcon name="back" /><span>กลับไปหน้าสัญญาว่าจ้าง</span></Link>{agreement.source_quotation_id ? <Link className="fee-agreement-navigation-link fee-agreement-navigation-source" style={{ ...navigationLink, ...navigationSourceLink }} href={`/finance/quotations/${agreement.source_quotation_id}`}><NavigationIcon name="source" /><span>เปิดใบเสนอราคาต้นทาง</span></Link> : null}</div><div className="fee-agreement-navigation-group fee-agreement-output-group" style={{ ...navigationGroup, ...outputGroup }} aria-label="ดูตัวอย่างและพิมพ์เอกสาร"><Link className="fee-agreement-navigation-link fee-agreement-navigation-preview" style={{ ...navigationLink, ...navigationPreviewLink }} href={`/finance/fee-agreements/${agreement.id}/preview`}><NavigationIcon name="preview" /><span>ดูตัวอย่าง</span></Link><Link className="fee-agreement-navigation-link fee-agreement-navigation-print" style={{ ...navigationLink, ...navigationPrintLink }} href={`/finance/fee-agreements/${agreement.id}/preview?print=1`}><NavigationIcon name="print" /><span>พิมพ์</span></Link></div></nav>{error ? <div style={warning}>{error}</div> : null}{message ? <div style={success}>{message}</div> : null}
    ...
    <section style={{ ...card, ...workflowPanel }}><h2 style={sectionTitle}>สถานะเอกสาร</h2>
      <div className="fee-agreement-workflow-overview" style={workflowOverview}>
        <div style={workflowStatusBlock}>
          ...
          <p style={workflowDescription}>{feeAgreementStatusDescription(agreement.status)}</p>
        </div>
      ...
      {permissions.canEditFinanceQuotation && agreement.status === "signed" ? <div className="fee-agreement-workflow-next" style={{ ...workflowNext, ...workflowFinanceNext }}>
        <div style={workflowNextCopy}><span style={{ ...workflowNextLabel, color: "#1d4ed8" }}>ขั้นตอนถัดไป</span><strong style={workflowNextTitle}>{billingPlan ? "ดำเนินการตามแผนเรียกเก็บเงิน" : "จัดทำแผนเรียกเก็บเงิน"}</strong><span style={workflowNextDescription}>{billingPlanDescription}</span>{billingPlan ? <span style={financePlanStatus}>สถานะแผน: {billingPlanStatusLabel(billingPlan.status)}</span> : null}</div>
        <div className="fee-agreement-workflow-primary-actions" style={workflowPrimaryActions}><button className="fee-agreement-finance-button" type="button" style={financeButton} disabled={billingPlanCreating} onClick={() => void openOrCreateBillingPlan()}><BillingPlanIcon />{billingPlanCreating ? "กำลังเตรียมแผน..." : billingPlan ? "เปิดแผนเรียกเก็บเงิน" : "สร้างแผนเรียกเก็บเงิน"}</button></div>
      </div> : null}
      {permissions.canEditFinanceQuotation && primaryLifecycleActions.length ? <div className="fee-agreement-workflow-next" style={workflowNext}>
        <div style={workflowNextCopy}><span style={workflowNextLabel}>ขั้นตอนถัดไป</span><strong style={workflowNextTitle}>{primaryLifecycleActions[0].label}</strong><span style={workflowNextDescription}>{feeAgreementActionDescription(primaryLifecycleActions[0].status)}</span></div>
      ...
      {permissions.canEditFinanceQuotation && signedClosureAction ? <div className="fee-agreement-workflow-secondary" style={workflowSecondary}>
        <div style={workflowSecondaryCopy}><span style={workflowDestructiveLabel}>การดำเนินการอื่น</span><span style={workflowSecondaryDescription}>ใช้เมื่อประสงค์ปิดเฉพาะกระบวนการเอกสาร การดำเนินการนี้ไม่ปิดงาน ไม่ยกเลิกแผนเรียกเก็บเงิน และไม่ถือว่าชำระเงินครบแล้ว</span></div>
        <button className="fee-agreement-workflow-secondary-button" type="button" style={workflowSecondaryButton} disabled={lifecycleSaving || paperSigningSaving || dirty} onClick={() => void changeStatus(signedClosureAction.status)}><WorkflowIcon name="completed" />{lifecycleSaving || paperSigningSaving ? "กำลังดำเนินการ..." : signedClosureAction.label}</button>
      </div> : null}
    </section>
    {permissions.canEditFinanceQuotation && agreement.status === "completed" ? <section className="fee-agreement-finance-panel" style={{ ...card, ...financePanel }}>
      <div className="fee-agreement-finance-panel-content" style={financePanelContent}>
        <div><span style={financeEyebrow}>งานด้านการเงิน</span><h2 style={financeTitle}>{billingPlan ? "ดำเนินการตามแผนเรียกเก็บเงิน" : "สร้างแผนเรียกเก็บเงิน"}</h2><p style={financeDescription}>{billingPlanDescription}</p>{billingPlan ? <span style={financePlanStatus}>สถานะแผน: {billingPlanStatusLabel(billingPlan.status)}</span> : null}</div>
  ...
  return <main style={page}>
    ...
    <nav className="fee-agreement-navigation-toolbar" style={navigationToolbar} aria-label="การนำทางรายการการว่าจ้าง">
      <div className="fee-agreement-navigation-group" style={navigationGroup}>
        <Link className="fee-agreement-navigation-link fee-agreement-navigation-back" style={{ ...navigationLink, ...navigationBackLink }} href="/finance/fee-agreements"><NavigationIcon name="back" /><span>กลับไปรายการการว่าจ้าง</span></Link>
        {agreement.source_quotation_id ? <Link className="fee-agreement-navigation-link fee-agreement-navigation-source" style={{ ...navigationLink, ...navigationSourceLink }} href={`/finance/quotations/${agreement.source_quotation_id}`}><NavigationIcon name="source" /><span>เปิดใบเสนอราคาต้นทาง</span></Link> : null}
      </div>
    </nav>
    ...
    <section style={{ ...card, ...acceptedEvidenceCard }}>
      ...
      <div style={grid}>
        ...
        <Field label="ช่องทางการยืนยัน" value={engagementChannelLabel(agreement.engagement_confirmation_channel)} />
        <Field label="ใบเสนอราคาต้นทาง" value={agreement.source_quotation_id ? <Link href={`/finance/quotations/${agreement.source_quotation_id}`}>{sourceQuotationNo}</Link> : sourceQuotationNo} />
...
function mapBillingPlanError(value: unknown) { const message = value && typeof value === "object" && "message" in value ? String(value.message) : String(value || ""); if (message.includes("signed, completed, or legacy active")) return "ข้อตกลงต้องอยู่ในสถานะลงนามแล้วหรือปิดกระบวนการเอกสารแล้วก่อนสร้างแผนเรียกเก็บเงิน"; if (message.includes("eligible commercial engagement")) return "รายการการว่าจ้างนี้ยังไม่พร้อมสร้างแผนเรียกเก็บเงิน กรุณารีเฟรชและตรวจสอบสถานะ"; if (message.includes("non-cancelled billing plan") || message.includes("uq_finance_billing_plans_active_agreement")) return "รายการนี้มีแผนเรียกเก็บเงินที่ยังไม่ถูกยกเลิกอยู่แล้ว กรุณารีเฟรชเพื่อเปิดแผนเดิม"; if (message.includes("VAT allocation") || message.includes("preserve each fee agreement item")) return "ยอดจัดสรรหรือ VAT ไม่ตรงกับหลักฐานการว่าจ้าง กรุณาตรวจสอบข้อมูลก่อนสร้างแผน"; if (message.includes("Not allowed")) return "คุณไม่มีสิทธิ์จัดการแผนเรียกเก็บเงิน"; return "ไม่สามารถสร้างแผนเรียกเก็บเงินได้ กรุณารีเฟรชและลองอีกครั้ง"; }
function mapAcceptedEngagementCancellationError(value: unknown) { const message = value && typeof value === "object" && "message" in value ? String(value.message) : String(value || ""); if (message.includes("Cancel the Billing Plan")) return "กรุณายกเลิกแผนเรียกเก็บเงินที่ยังมีผลก่อนยกเลิกการว่าจ้าง"; if (message.includes("Only a confirmed")) return "รายการนี้ไม่อยู่ในสถานะที่ยกเลิกได้ กรุณารีเฟรชหน้า"; if (message.includes("Not allowed")) return "คุณไม่มีสิทธิ์ยกเลิกการว่าจ้างรายการนี้"; return "ยกเลิกการว่าจ้างไม่สำเร็จ กรุณารีเฟรชและลองอีกครั้ง"; }
function engagementChannelLabel(value: string | null) { return ({ line: "LINE", email: "Email", phone: "โทรศัพท์", meeting: "นัด/ประชุม", written: "หนังสือ/ข้อความเป็นลายลักษณ์อักษร", other: "อื่น ๆ" } as Record<string, string>)[value || ""] || "-"; }
...
function billingPlanNextStepDescription(plan: BillingPlanReference | null, acceptedQuotationBasis = false) {
  if (!plan) return acceptedQuotationBasis ? "กำหนดงวดและเงื่อนไขการเรียกเก็บเงินตามใบเสนอราคาที่ตอบรับ การสร้างแผนยังไม่เป็นการออกใบแจ้งหนี้" : "กำหนดงวดและเงื่อนไขการเรียกเก็บเงินตามข้อตกลงค่าบริการ การสร้างแผนยังไม่เป็นการออกใบแจ้งหนี้";
  if (plan.status === "draft") return "ตรวจสอบและบันทึกกำหนดงวด เงื่อนไข และยอดจัดสรรให้ครบ ก่อนยืนยันว่าแผนพร้อมดำเนินการ";
  if (plan.status === "active") return "แผนพร้อมดำเนินการเรียกเก็บเงินแล้ว เปิดแผนเพื่อตรวจสอบงวดและดำเนินงาน Finance ขั้นถัดไป";
  if (plan.status === "completed") return "แผนเรียกเก็บเงินดำเนินการครบแล้ว เปิดแผนเพื่อตรวจสอบรายละเอียดและประวัติการดำเนินงาน";
  return "เปิดแผนเรียกเก็บเงินเพื่อตรวจสอบสถานะ งวด และรายละเอียดการดำเนินงาน";
}
