...
function Detail({ permissions }: { permissions: { canEditFinanceQuotation: boolean } }) {
  ...
  return <main style={page}><FinanceSubNav activePage="fee-agreements" permissions={permissions as never} /><nav className="fee-agreement-navigation-toolbar" style={navigationToolbar} aria-label="การนำทางเอกสาร"><Link className="fee-agreement-navigation-link fee-agreement-navigation-back" style={{ ...navigationLink, ...navigationBackLink }} href="/finance/fee-agreements"><NavigationIcon name="back" /><span>กลับไปหน้าสัญญาว่าจ้าง</span></Link>{agreement.source_quotation_id ? <Link className="fee-agreement-navigation-link fee-agreement-navigation-source" style={{ ...navigationLink, ...navigationSourceLink }} href={`/finance/quotations/${agreement.source_quotation_id}`}><NavigationIcon name="source" /><span>เปิดใบเสนอราคาต้นทาง</span></Link> : null}<Link className="fee-agreement-navigation-link fee-agreement-navigation-preview" style={{ ...navigationLink, ...navigationPreviewLink }} href={`/finance/fee-agreements/${agreement.id}/preview`}><NavigationIcon name="preview" /><span>ดูตัวอย่าง / พิมพ์</span></Link></nav>{error ? <div style={warning}>{error}</div> : null}{message ? <div style={success}>{message}</div> : null}
  ...
  <style jsx global>{`
    ...
    .fee-agreement-navigation-preview:hover { background: #26334c !important; border-color: #26334c !important; }
    ...
    @media (max-width: 720px) {
      .fee-agreement-navigation-toolbar { grid-template-columns: minmax(0, 1fr) !important; }
...
function NavigationIcon({ name }: { name: "back" | "source" | "preview" }) { const common = { width: 17, height: 17, viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: 2, strokeLinecap: "round" as const, strokeLinejoin: "round" as const, "aria-hidden": true }; if (name === "back") return <svg {...common}><path d="M19 12H5M12 19l-7-7 7-7" /></svg>; if (name === "source") return <svg {...common}><path d="M6 3h9l3 3v15H6zM14 3v4h4M9 12h6M9 16h4" /></svg>; return <svg {...common}><path d="M6 9V3h12v6M6 18H4a2 2 0 0 1-2-2v-5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2h-2M6 14h12v7H6z" /></svg>; }
...
const navigationToolbar: CSSProperties = { display: "grid", gridTemplateColumns: "repeat(3, max-content)", alignItems: "center", gap: 8, padding: 8, marginBottom: 18, border: "1px solid #e2e8f0", borderRadius: 8, background: "#f8fafc" };
...
const navigationPreviewLink: CSSProperties = { background: "#172033", borderColor: "#172033", color: "#fff" };
