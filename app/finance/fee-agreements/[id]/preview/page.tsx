...
function Preview({ id }: { id: string }) {
  ...
  const primaryClientSigner = signatories.find((entry) => entry.party_type === "client") || null;
...
function PreambleRenderer({ section: templateSection, variables, agreement, snapAgreement, client, matter, source, documentTitle, identity, logoUrl, clientSigner }: { section?: Json; variables: Record<string, string>; agreement: Agreement; snapAgreement: Json; client: Json; matter: Json; source: Json; documentTitle: string; identity: DocumentIdentity; logoUrl: string; clientSigner: FeeAgreementSignatory | null }) {
  ...
  const clientName = variableValue(variables, "CLIENT_NAME", string(client.name, string(client.display_name)));
  const signerContext = clientSigner ? feeAgreementSignatoryContext(clientSigner, clientName) : "";
  const clientRows: Array<[string, string]> = [["ชื่อ", clientName], ["เลขประจำตัวผู้เสียภาษี", variableValue(variables, "CLIENT_TAX_ID", string(client.tax_id))], ["ที่อยู่", variableValue(variables, "CLIENT_ADDRESS", string(client.address, string(client.address_th)))], ["ผู้ลงนาม", variableValue(variables, "CLIENT_SIGNATORY_NAME")], ["ตำแหน่ง/ฐานะ", variableValue(variables, "CLIENT_SIGNATORY_TITLE")]];
  if (signerContext) clientRows.push(["ฐานะการลงนาม", signerContext]);
  return <section data-section-kind="preamble" data-section-code={string(templateSection?.section_code, "PREAMBLE")} style={preambleSection}>
    ...
    <div style={infoGrid}>
      <Info title="ผู้รับบริการ / ลูกค้า" rows={clientRows} />
