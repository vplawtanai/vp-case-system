...
function Preview({ id }: { id: string }) {
  ...
  const load = useCallback(async () => {
    ...
    setAgreement(row); setItems((itemRes.data || []) as Item[]); setLiveTemplate(asObject(templateRes.data)); setDocumentIdentity(identity); setLogoUrl(resolvedLogoUrl); setLoading(false);
  }, [id]);
  ...
  return <main style={shell}><div className="screen-controls" style={controls}><Link href={`/finance/fee-agreements/${agreement.id}`}>Back to Fee Agreement</Link><button type="button" onClick={() => window.print()}>Print</button><span>เพื่อเอกสารที่สะอาด กรุณาปิด Headers and footers ในหน้าต่าง Print</span></div>{previewIncomplete ? <div className="fee-agreement-preview-readiness" style={readinessNotice}>เอกสารฉบับนี้ยังอยู่ระหว่างจัดทำและอาจมีข้อมูลไม่ครบถ้วน</div> : null}
    <article className="fee-agreement-document" style={documentStyle}><PreambleRenderer section={rendererV3 ? preambleSection : undefined} variables={variables} agreement={agreement} snapAgreement={snapAgreement} client={client} matter={matter} source={source} documentTitle={documentTitle} identity={documentIdentity} logoUrl={logoUrl} />
      ...
      <ExecutionRenderer section={rendererV3 ? executionSection : undefined} requirements={signatureRequirements} signatories={signatories} variables={variables} />
...
function resolvedVariableMap(value: unknown): Record<string, string> { if (!Array.isArray(value)) return {}; return value.reduce<Record<string, string>>((result, entry) => { const row = asObject(entry); const key = string(row.variable_key, ""); if (!key || row.resolution_result === "unresolved") return result; const formatted = string(row.formatted_value, ""); const raw = string(row.value, ""); result[key] = formatted || raw; return result; }, {}); }
function variableValue(variables: Record<string, string>, key: string, fallback = "-") { return variables[key] || fallback; }
function PreambleRenderer({ section: templateSection, variables, agreement, snapAgreement, client, matter, source, documentTitle, identity, logoUrl }: { section?: Json; variables: Record<string, string>; agreement: Agreement; snapAgreement: Json; client: Json; matter: Json; source: Json; documentTitle: string; identity: DocumentIdentity; logoUrl: string }) {
  const agreementNo = variableValue(variables, "AGREEMENT_NO", string(snapAgreement.agreement_no, agreement.agreement_no || "-"));
  const matterName = variableValue(variables, "MATTER_NAME", string(matter.title, string(matter.name, string(matter.file_no, string(matter.matter_no)))));
  return <section data-section-kind="preamble" data-section-code={string(templateSection?.section_code, "PREAMBLE")} style={preambleSection}>
    <DocumentIdentityHeader className="fee-agreement-document-header" identity={identity} logoUrl={logoUrl} title={variableValue(variables, "AGREEMENT_TITLE", documentTitle)} subtitle="Fee Agreement" documentNo={agreementNo} />
    <div style={infoGrid}>
      <Info title="ผู้รับบริการ / ลูกค้า" rows={[["ชื่อ", variableValue(variables, "CLIENT_NAME", string(client.name, string(client.display_name)))], ["เลขประจำตัวผู้เสียภาษี", variableValue(variables, "CLIENT_TAX_ID", string(client.tax_id))], ["ที่อยู่", variableValue(variables, "CLIENT_ADDRESS", string(client.address, string(client.address_th)))], ["ผู้ลงนาม", variableValue(variables, "CLIENT_SIGNATORY_NAME")], ["ตำแหน่ง/ฐานะ", variableValue(variables, "CLIENT_SIGNATORY_TITLE")]]} />
      <Info title="ข้อมูลเอกสาร" rows={[["สถานะ", string(snapAgreement.status, agreement.status)], ["วันที่สัญญา", variableValue(variables, "AGREEMENT_DATE", date(agreement.created_at))], ["วันที่สัญญามีผล", variableValue(variables, "EFFECTIVE_DATE", date(snapAgreement.effective_date || agreement.effective_date))], ["เรื่อง/คดี", matterName], ["ใบเสนอราคาต้นทาง", variableValue(variables, "SOURCE_QUOTATION_NO", string(source.quotation_no))]]} />
    </div>
  </section>;
}
...
function ExecutionRenderer({ section: templateSection, requirements, signatories, variables }: { section?: Json; requirements: Json; signatories: FeeAgreementSignatory[]; variables: Record<string, string> }) {
  ...
  return <section data-section-kind="execution" data-section-code={string(templateSection?.section_code, "EXECUTION")} style={section}>
    <SectionTitle>{string(templateSection?.title, "การลงนาม")}</SectionTitle>
    <SignatoryList rows={withFallbacks} />
    <div style={signatureGrid}>{firm.length || minimumFirm ? signatureSlots("ผู้ให้บริการ / สำนักงาน", firm, minimumFirm) : null}{client.length || minimumClient ? signatureSlots("ผู้รับบริการ / ลูกค้า", client, minimumClient) : null}{witness.length || minimumWitness ? signatureSlots("พยาน", witness, minimumWitness) : null}</div>
  </section>;
}
function signatureSlots(title: string, rows: FeeAgreementSignatory[], minimum: number) { const entries: Array<FeeAgreementSignatory | null> = rows.length ? rows : Array.from({ length: Math.max(minimum, 1) }, () => null); return entries.map((signer, index) => <Signature key={`${title}-${index}`} title={entries.length > 1 ? `${title} ${index + 1}` : title} signer={signer} />); }
function SignatoryList({ rows }: { rows: FeeAgreementSignatory[] }) { const entries = [...rows].filter((row) => row.name).sort((left, right) => left.sort_order - right.sort_order); return entries.length ? <div>{entries.map((row, index) => <div key={`${row.name}-${index}`} style={term}><strong>{row.name}</strong><div>{row.capacity || "-"} · {partyLabel[row.party_type] || "ไม่ระบุฝ่าย"}</div></div>)}</div> : <p style={muted}>ยังไม่มีข้อมูลผู้ลงนาม</p>; }
function Signature({ title, signer }: { title: string; signer: FeeAgreementSignatory | null }) { return <div style={signature}><strong>{title}</strong><div style={line} /><div>ชื่อ: {signer?.name || "-"}</div><div>ตำแหน่ง/ฐานะ: {signer?.capacity || "-"}</div><div>วันที่: ____________________</div></div>; }
