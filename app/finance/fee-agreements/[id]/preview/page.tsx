...
function ExecutionRenderer({ section: templateSection, requirements, signatories, variables, signatureUrls, clientName, languageCode, executionMode, legacyFrozenExecution }: { section?: Json; requirements: Json; signatories: FeeAgreementSignatory[]; variables: Record<string, string>; signatureUrls: Record<string, string>; clientName: string; languageCode: string; executionMode: FeeAgreementExecutionMode; legacyFrozenExecution: boolean }) {
  ...
  return <section className="fee-agreement-execution" data-section-kind="execution" data-section-code={string(templateSection?.section_code, "EXECUTION")} style={section}>
    {legacyFrozenExecution ? <><SectionTitle>{string(templateSection?.title, "การลงนาม")}</SectionTitle><SignatoryList rows={withFallbacks} clientName={clientName} /></> : <p style={executionAttestation}>{feeAgreementClosingCopy({ mode: executionMode, hasWitnesses: witness.length > 0 || minimumWitness > 0, languageCode })}</p>}
    <div style={signatureGrid}>{client.length || minimumClient ? signatureSlots("ผู้ว่าจ้าง", client, minimumClient, legacyFrozenExecution ? signatureUrls : {}, clientName, languageCode) : null}{firm.length || minimumFirm ? signatureSlots("ผู้ให้บริการ", firm, minimumFirm, legacyFrozenExecution ? signatureUrls : {}, clientName, languageCode) : null}{witness.length || minimumWitness ? signatureSlots("พยาน", witness, minimumWitness, legacyFrozenExecution ? signatureUrls : {}, clientName, languageCode) : null}</div>
  </section>;
}
