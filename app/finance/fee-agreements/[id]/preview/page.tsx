...
function Preview({ id }: { id: string }) {
  ...
  return <main style={shell}><div className="screen-controls" style={controls}><Link href={`/finance/fee-agreements/${agreement.id}`}>Back to Fee Agreement</Link><button type="button" onClick={() => window.print()}>Print</button><span>เพื่อเอกสารที่สะอาด กรุณาปิด Headers and footers ในหน้าต่าง Print</span></div>{error ? <div className="fee-agreement-preview-readiness" style={readinessNotice}>{error}</div> : null}{previewIncomplete ? <div className="fee-agreement-preview-readiness" style={readinessNotice}>เอกสารฉบับนี้ยังอยู่ระหว่างจัดทำและอาจมีข้อมูลไม่ครบถ้วน</div> : null}
    ...
    </article><style jsx global>{`@page { size: A4 portrait; margin: 11mm; } @media print { body { background: #fff !important; } .screen-controls, .fee-agreement-preview-readiness, nav, header:not(.fee-agreement-document-header), aside, button, [role="dialog"] { display: none !important; } .fee-agreement-document { box-shadow: none !important; border: 0 !important; max-width: none !important; margin: 0 !important; padding: 0 !important; font-size: 10pt !important; line-height: 1.58 !important; } .fee-agreement-document table { width: 100% !important; } .fee-agreement-document tr, .fee-agreement-summary, .fee-agreement-principal-signatures, .fee-agreement-signature { break-inside: avoid; page-break-inside: avoid; } .fee-agreement-execution-attestation { break-after: avoid-page; page-break-after: avoid; } .fee-agreement-witness-signatures { break-inside: auto; page-break-inside: auto; } .fee-agreement-section-title { break-after: avoid; page-break-after: avoid; } }`}</style></main>;
}
...
function ExecutionRenderer({ section: templateSection, requirements, signatories, variables, signatureUrls, clientName, languageCode, executionMode, legacyFrozenExecution }: { section?: Json; requirements: Json; signatories: FeeAgreementSignatory[]; variables: Record<string, string>; signatureUrls: Record<string, string>; clientName: string; languageCode: string; executionMode: FeeAgreementExecutionMode; legacyFrozenExecution: boolean }) {
  ...
  return <section className="fee-agreement-execution" data-section-kind="execution" data-section-code={string(templateSection?.section_code, "EXECUTION")} style={section}>
    {legacyFrozenExecution ? <><SectionTitle>{string(templateSection?.title, "การลงนาม")}</SectionTitle><SignatoryList rows={withFallbacks} clientName={clientName} /></> : <p className="fee-agreement-execution-attestation" style={executionAttestation}>{feeAgreementClosingCopy({ mode: executionMode, hasWitnesses: witness.length > 0 || minimumWitness > 0, languageCode })}</p>}
    <div className="fee-agreement-principal-signatures" style={signatureGrid}>{client.length || minimumClient ? signatureSlots("ผู้ว่าจ้าง", client, minimumClient, legacyFrozenExecution ? signatureUrls : {}, clientName, languageCode) : null}{firm.length || minimumFirm ? signatureSlots("ผู้ให้บริการ", firm, minimumFirm, legacyFrozenExecution ? signatureUrls : {}, clientName, languageCode) : null}</div>
    {witness.length || minimumWitness ? <div className="fee-agreement-witness-signatures" style={witnessSignatureGrid}>{signatureSlots("พยาน", witness, minimumWitness, legacyFrozenExecution ? signatureUrls : {}, clientName, languageCode)}</div> : null}
  </section>;
}
