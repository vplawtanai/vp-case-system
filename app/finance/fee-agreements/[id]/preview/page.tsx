...
function Preview({ id }: { id: string }) {
  ...
  const load = useCallback(async () => {
    ...
    if (header.error || !header.data) { setError(uiMessage("finance.feeAgreement.workspace.error.load")); setLoading(false); return; }
    ...
    if (itemRes.error) setError(uiMessage("finance.feeAgreement.workspace.preview.itemsLoad"));
  ...
  if (loading) return <main style={shell}>{t("finance.feeAgreement.workspace.preview.load")}</main>; if (!agreement) return <main style={shell}>{uiText(error) || t("finance.feeAgreement.workspace.error.notFound")}</main>;
  if (agreement.engagement_basis === "accepted_quotation") return <main style={shell}><div style={acceptedQuotationPreviewNotice}><h1 style={acceptedQuotationPreviewTitle}>{t("finance.feeAgreement.workspace.preview.noAgreement")}</h1><p style={acceptedQuotationPreviewCopy}>{t("finance.feeAgreement.workspace.preview.acceptedHelp")}</p><div style={acceptedQuotationPreviewActions}><Link href={`/finance/fee-agreements/${agreement.id}`} style={acceptedQuotationPreviewBack}>{t("finance.feeAgreement.workspace.preview.backEngagement")}</Link>{agreement.source_quotation_id ? <Link href={`/finance/quotations/${agreement.source_quotation_id}/preview`} style={acceptedQuotationPreviewSource}>{t("finance.invoice.ui.openQuotation")}</Link> : null}</div></div></main>;
  ...
  return <main style={shell}><div className="screen-controls fee-agreement-preview-toolbar" style={controls}><div style={controlMessage}><PreviewControlIcon name="info" /><div><strong style={controlTitle}>{t("finance.feeAgreement.workspace.preview.review")}</strong><span style={controlText}>{t("finance.feeAgreement.workspace.preview.printHelp")}</span></div></div><div className="fee-agreement-preview-actions" style={controlActions}><Link className="fee-agreement-preview-button fee-agreement-preview-back" href={`/finance/fee-agreements/${agreement.id}`} style={{ ...controlButton, ...controlBackButton }}><PreviewControlIcon name="back" />{t("finance.feeAgreement.workspace.preview.back")}</Link><button className="fee-agreement-preview-button fee-agreement-preview-print" type="button" style={{ ...controlButton, ...controlPrintButton }} onClick={() => { void printWhenReady(); }}><PreviewControlIcon name="print" />{t("finance.invoice.ui.print")}</button></div></div>{error ? <div className="fee-agreement-preview-readiness" style={readinessNotice}>{uiText(error)}</div> : null}{previewIncomplete ? <div className="fee-agreement-preview-readiness" style={readinessNotice}>{t("finance.feeAgreement.workspace.preview.incomplete")}</div> : null}
    ...
    </LegalDocumentLayout><style jsx global>{`
...
const pre = { whiteSpace: "pre-wrap" as const, lineHeight: 1.68, margin: "7px 0", orphans: 3, widows: 3 };
const term = { padding: "8px 0", borderBottom: "1px solid #e5e7eb" };
const tableWrap = { overflow: "visible" as const };
const table = { width: "100%", minWidth: 0, tableLayout: "fixed" as const, borderCollapse: "collapse" as const, fontSize: 12 };
const feeCell = { padding: "8px 6px", borderBottom: "1px solid #e5e7eb", verticalAlign: "top" as const };
const feeHeaderCell = { background: "#f8fafc", color: "#334155", fontWeight: 700, borderTop: "1px solid #cbd5e1", borderBottom: "1px solid #94a3b8" };
const summary = { marginLeft: "auto", marginTop: 14, maxWidth: 380, background: "#f0fdf4", border: "1px solid #bbf7d0", padding: 14, display: "grid", gap: 8, textAlign: "right" as const, breakInside: "avoid" as const };
const grand = { borderTop: "1px solid #86efac", paddingTop: 8, fontSize: 16, color: "#166534" };
const executionAttestation = { margin: 0, lineHeight: 1.72, textAlign: "start" as const, overflowWrap: "normal" as const, orphans: 3, widows: 3, breakAfter: "avoid-page" as const };
const signatureGrid = { display: "grid", gridTemplateColumns: "repeat(2,minmax(0,1fr))", gap: 26, marginTop: 20 };
const witnessSignatureGrid = { ...signatureGrid, marginTop: 24 };
const signature = { minWidth: 0, minHeight: 130, borderTop: "1px solid #94a3b8", paddingTop: 10, fontSize: 13, lineHeight: 1.8, breakInside: "avoid" as const };
const signatureMark = { height: 54, display: "flex", alignItems: "flex-end" };
const signatureImage = { display: "block", maxWidth: 150, maxHeight: 50, objectFit: "contain" as const, objectPosition: "left bottom" };
const line = { borderBottom: "1px solid #64748b", marginBottom: 8 };
const paymentGrid = { display: "grid", gap: 12 };
const paymentGridStyle = (installmentCount: number) => ({ ...paymentGrid, gridTemplateColumns: `repeat(${Math.min(Math.max(installmentCount, 1), 3)},minmax(0,1fr))` });
const paymentCard = { minWidth: 0, border: "1px solid #e5e7eb", borderRadius: 6, padding: 12, lineHeight: 1.6, breakInside: "avoid" as const };
const allocated = { display: "flex", justifyContent: "space-between", gap: 10, padding: "3px 0", borderBottom: "1px solid #f1f5f9" };
const commercialSupplement = { marginTop: 16, paddingTop: 12, borderTop: "1px solid #e5e7eb" };
const supplementTitle = { margin: "14px 0 8px", fontSize: 14, color: "#334155", breakAfter: "avoid" as const };
