...
const statusDescriptions: Record<string, string> = {
  ...
  sent: "ระบบบันทึกว่าเอกสารถูกส่งให้ลูกค้าผ่านช่องทางภายนอกแล้ว เอกสารเป็นแบบอ่านอย่างเดียวและรอการลงนาม",
...
const actionDescriptions: Record<FeeAgreementLifecycleTarget, string> = {
  ...
  sent: "ใช้เมื่อได้ส่งเอกสารให้ลูกค้าผ่านช่องทางภายนอกแล้ว ระบบจะไม่ส่งเอกสารให้อัตโนมัติ",
  signed: "บันทึกสถานะว่าเอกสารได้รับการลงนามแล้ว โดยระบบยังไม่ตรวจสอบลายมือชื่ออิเล็กทรอนิกส์",
...
const actionLabels: Record<FeeAgreementLifecycleTarget, string> = {
  ...
  sent: "บันทึกว่าส่งให้ลูกค้าแล้ว",
...
const confirmations: Record<FeeAgreementLifecycleTarget, string> = {
  ...
  sent: "ยืนยันว่าได้ส่งเอกสารให้ลูกค้าผ่านช่องทางภายนอกแล้ว?\n\nระบบจะบันทึกสถานะว่า “ส่งแล้ว” และเก็บข้อมูลเอกสารฉบับที่ส่งไว้เป็นหลักฐาน ระบบจะไม่ส่งเอกสารให้ลูกค้าโดยอัตโนมัติ",
...
export function feeAgreementLifecycleSuccess(status: FeeAgreementLifecycleTarget) {
  if (status === "sent") return "บันทึกว่าส่งเอกสารให้ลูกค้าผ่านช่องทางภายนอกแล้ว และบันทึกเวอร์ชันเอกสารแล้ว";
