...
test("additional-only V2 recognizes service and recoverable-cost Charges", () => {
  for (const items of [[item("ad_hoc_service")], [item("recoverable_cost")], [item("ad_hoc_service"), item("recoverable_cost")]]) {
    assert.equal(invoiceCompositionSourceLabel("billable_charge_v2", items), "รายการเรียกเก็บนอกใบเสนอราคา");
  }
});
...
test("mixed V2 requires both effective source types", () => {
  for (const additionalType of ["ad_hoc_service", "recoverable_cost"]) {
    ...
    assert.equal(invoiceCompositionSourceLabel("billable_charge_v2", items), "ยอดตามแผน + รายการเรียกเก็บนอกใบเสนอราคา");
    assert.equal(invoiceCompositionSourceLabel("billable_charge_v2", [...items].reverse()), "ยอดตามแผน + รายการเรียกเก็บนอกใบเสนอราคา");
  }
});
...
test("released composition history cannot make an installment-only Draft look mixed", () => {
  ...
  assert.equal(invoiceCompositionSourceLabel("billable_charge_v2", items), "ยอดตามแผนเรียกเก็บเงิน");
  assert.equal(invoiceCompositionSourceLabel("billable_charge_v2", [item("billing_installment_item", "released"), item("ad_hoc_service")]), "รายการเรียกเก็บนอกใบเสนอราคา");
});
