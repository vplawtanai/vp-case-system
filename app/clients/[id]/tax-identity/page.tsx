...
export default function CustomerTaxIdentityPage() {
  ...
  return <AuthGuard><AppTopNav title={t("client.tax.title")} activePage="clients" /><CustomerTaxIdentityEditor key={id} /></AuthGuard>;
}
export function CustomerTaxIdentityEditor() {
  const { id } = useParams<{ id: string }>();
  ...
  return <main className={styles.page}>
    <Link href="/clients" className={styles.button}>{t("client.tax.back")}</Link>
    <header className={styles.header}><h1>{t("client.tax.title")}</h1>{result ? <p>{result.identity.name || "-"}</p> : null}</header>
    ...
    {!result || !form ? (!error ? <p>{t("common.state.loading")}</p> : null) : <form onSubmit={save} noValidate>
      ...
      </dl><Link href="/clients">{t("client.tax.editClient")}</Link></section>
