...
// Candidate 061 checks exact prior/after functions, unchanged catalog and business evidence hashes.
const fs=require('node:fs'),assert=require('node:assert/strict'),{createHash}=require('node:crypto');
const prior=require('./company-review-modal-artifacts.cjs');
const migrationPath='supabase/migrations/202607180061_add_employee_reimbursement_review.sql';
const manifestPath='scripts/tests/employee-reimbursement-catalog.json';
const filenames={pre:'scripts/sql/preflight_employee_reimbursement.sql',dry:'scripts/sql/dry_run_employee_reimbursement.sql',verify:'scripts/sql/verify_employee_reimbursement.sql'};
const source=()=>fs.readFileSync(migrationPath,'utf8'),sha=()=>createHash('sha256').update(source()).digest('hex'),q=s=>"'"+s.replaceAll("'","''")+"'";
const changed=['get_finance_expense_access'];
const added=['review_finance_employee_reimbursement'];
...
checks(name,passed) as(values
 ('exact_${post?'061':'060'}_and_preserved_functions',not exists(select 1 from function_differences)),
 ...
 ('rehearsal_upstream_unchanged',nullif(current_setting('vp.expense061_before',true),'') is null or current_setting('vp.expense061_before',true)=(select hashes::text from protected)))
select (select jsonb_object_agg(name,passed is true) from checks) checks,
 ...
 (select bool_and(passed is true) from checks) employee_reimbursement_${post?'verification':'preflight'}_pass,
 (select hashes from protected) upstream_evidence_hashes,
 nullif(current_setting('vp.expense061_before',true),'') is not null rehearsal_baseline_available,
 ...
 ${q(sha())} migration_061_sha256,current_setting('server_version_num')::int catalog_server_version_num;
`;
...
BEGIN;
select set_config('vp.expense061_before',upstream_evidence_hashes::text,true),set_config('vp.expense061_preflight',employee_reimbursement_preflight_pass::text,true) from (${pre.trim().replace(/;$/,'')}) p;
DO $gate$ BEGIN IF current_setting('vp.expense061_preflight')<>'true' THEN RAISE EXCEPTION '061 preflight failed; STOP'; END IF; END $gate$;
-- BEGIN EMBEDDED MIGRATION 061
${source()}-- END EMBEDDED MIGRATION 061
${verify}ROLLBACK;
`;
...
if(require.main===module){for(const [file,sql] of Object.entries(workflow())){if(process.argv.includes('--write'))fs.writeFileSync(file,sql);else assert.equal(fs.readFileSync(file,'utf8'),sql);}console.log('061 artifacts verified');}
