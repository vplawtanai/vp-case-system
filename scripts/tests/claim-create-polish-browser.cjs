...
module.exports=async({page,url,out,translate})=>{
 ...
 for(const[width,locale]of[[390,'th'],[1440,'th'],[1440,'en']]){
  ...
  assert.equal(await dialog.getByRole('option').count(),21);assert.equal(await dialog.getByRole('option').last().innerText(),locale==='th'?'อื่น ๆ':'Other');
