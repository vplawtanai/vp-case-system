...
function fixture(future=false){
 ...
 const pools=['vat','wht_natural','wht_juristic'].map(filing_type=>({period_month:'2026-09-01',filing_type,source_count:filing_type==='vat'?1:future?1:0,base_amount:filing_type==='vat'?10000:future?3104:0,tax_amount:filing_type==='vat'?null:future?93.12:0,output_vat:filing_type==='vat'?700:null,input_vat:null,input_vat_complete:false,ready:filing_type!=='vat',sources:filing_type==='vat'?[]:future?[{...source,id:id(filing_type==='wht_natural'?1:3),entity_type:filing_type==='wht_natural'?'natural_person':'juristic_person'}]:[],issues:filing_type==='vat'?[{code:'input_vat_incomplete',count:1}]:[],fingerprint:'synthetic'}));
 pools[0].issues[0].count=null;
 return{can_manage:true,can_remit:true,period_month:'2026-09-01',pools,filings:[],incoming_wht_credit:300,history:[],accounts:[{kind:'bank',account_id:id(8),bank_account_id:id(8),cash_location_id:null,name_th:'KBANK',name_en:'KBANK',is_active:true,currency:'THB',opening_id:id(9),opening_as_of:'2026-08-31T16:59:59.999999Z',opening_amount:10000,inflow:0,outflow:0,system_balance:10000}]};
}
