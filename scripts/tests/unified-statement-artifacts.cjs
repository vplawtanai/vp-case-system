...
const {candidatePath,contractPath}=require('./unified-statement-contract.cjs');
...
const files={preflight:'scripts/sql/preflight_unified_statement_070.sql',dryRun:'scripts/sql/dry_run_unified_statement_070.sql',verifier:'scripts/sql/verify_unified_statement_070.sql'};
const pinsPath='scripts/tests/unified-statement-approved-hashes.json';
...
function preflight(c){return `-- 070 SELECT-only Preflight. Copy this entire file into Supabase SQL Editor.
...
WITH ${cte(c,false)}, checks AS (select * from (values
 ...
 ('target_tables_absent',not exists(select 1 from pg_class where relnamespace='public'::regnamespace and relname in (${c.scope.newTables.map(q).join(',')}))),
...
`;}
function normalize(c,actual){
 const remove=Object.keys(c.after.functions).filter(s=>c.scope.created.includes(s.split('(')[0]));
 return `${actual}||jsonb_build_object('functions',(${actual}->'functions')-ARRAY[${remove.map(q).join(',')}]::text[],'catalog',((${actual}->'catalog')-ARRAY[${c.scope.newTables.map(q).join(',')}]::text[])||jsonb_build_object('finance_cash_transactions',${q(c.before.catalog.finance_cash_transactions)}))`;
}
const emptyTargets=c=>c.scope.newTables.map(t=>`not exists(select 1 from public.${t})`).join(' and ');
function verifier(c,pins){return `-- 070 SELECT-only Post-Apply Verifier. No DDL/DML or mutable RPC calls.
-- Fail closed until the Production PASS Preflight state/row hashes are pinned.
-- Normalization replaces only the exact verified 070 function delta in memory.
WITH ${cte(c,true)}, normalized AS(select ${normalize(c,'value')} value from actual),checks AS(select * from (values
 ...
 ('frozen_share_projection',${projectionCheck}),('no_migration_postings',${emptyTargets(c)}),
...
FROM checks;
`;}
function dryRun(c,pins){return `-- 070 ROLLBACK-ONLY. Run COMPLETE file; any SQL error is a failed gate.
...
DECLARE actual jsonb; baseline jsonb; failures jsonb; expected jsonb;
 capture_sql text:=${q(prior.captureSql(c))}; candidate_sql text:=$candidate070$${source()}$candidate070$;
BEGIN
 IF ${hash('candidate_sql')}<>${q(c.candidateSha)} THEN RAISE EXCEPTION '070_CANDIDATE_HASH_MISMATCH'; END IF;
 EXECUTE capture_sql INTO baseline;
 IF ${hash('baseline')}<>${q(pins.stateSha)} OR ${hash("baseline->'rows'")}<>${q(pins.rowsSha)} THEN RAISE EXCEPTION '070_APPROVED_PREFLIGHT_REQUIRED_OR_BASELINE_CHANGED'; END IF;
 ...
 IF failures<>'[]'::jsonb OR NOT (${policyCheck}) OR NOT (${prior.v2Check}) THEN RAISE EXCEPTION '070_PRECONDITION_FAILED: %',failures; END IF;
 ...
 ${prior.differences('actual','expected')} INTO failures;
 IF failures<>'[]'::jsonb OR NOT (${policyCheck}) OR NOT (${prior.v2Check}) OR NOT (${projectionCheck}) OR NOT (${emptyTargets(c)}) THEN RAISE EXCEPTION '070_POSTCONDITION_FAILED: %',failures; END IF;
END;
$gate070$;
ROLLBACK;
...
function validate({preflightOnly=false}={}){const c=JSON.parse(fs.readFileSync(contractPath)),{manifestSha,...body}=c,pins=JSON.parse(fs.readFileSync(pinsPath));assert.equal(fingerprint(body),manifestSha);assert.equal(sha(source()),c.candidateSha);for(const [p,h] of Object.entries(c.appliedMigrationHashes))assert.equal(sha(fs.readFileSync(p)),h,p);const selected=preflightOnly?{[files.preflight]:preflight(c)}:artifacts(c,pins);for(const [file,sql] of Object.entries(selected)){assert.equal(fs.readFileSync(file,'utf8'),sql,file);assert.ok(Buffer.byteLength(sql)<250000,file+' too large');}return c;}
if(require.main===module){const c=JSON.parse(fs.readFileSync(contractPath)),pins=JSON.parse(fs.readFileSync(pinsPath)),mode=process.argv[2],preflightOnly=['--write-preflight','--validate-preflight'].includes(mode);if(mode==='--write')for(const [file,sql] of Object.entries(artifacts(c,pins)))fs.writeFileSync(file,sql);if(mode==='--write-preflight')fs.writeFileSync(files.preflight,preflight(c));validate({preflightOnly});console.log('070 candidate / applied migration hashes / '+(preflightOnly?'SELECT-only Preflight':'static gates')+' PASS '+c.candidateSha);}
