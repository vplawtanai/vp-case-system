...
const migrationPath='supabase/migrations/202607180057_add_company_expense_payment_fact.sql';
const manifestPath='scripts/tests/expense-payment-fact-catalog.json';
const filenames={pre:'scripts/sql/preflight_expense_payment_fact.sql',dry:'scripts/sql/dry_run_expense_payment_fact.sql',verify:'scripts/sql/verify_expense_payment_fact.sql'};
const tables=['finance_expenses'];
...
const priorSql=()=>functionSql([...created(),...preserved]);
...
function workflow(){
 ...
 (select bool_and(passed is true) from checks) expense_payment_fact_${post?'verification':'preflight'}_pass,
 ...
 ${q(sha())} migration_057_sha256,current_setting('server_version_num')::int catalog_server_version_num;\n`;
 ...
 const rpcs=['get_finance_expense_access'];
 ...
 ('no_historical_backfill',not exists(select 1 from finance_expenses where creator_payment_fact is not null)),
 ('browser_access_blocked',(select count(*)=1 and bool_and(relrowsecurity and not has_table_privilege('authenticated',oid,'INSERT,UPDATE,DELETE,TRUNCATE,REFERENCES,TRIGGER') and not has_any_column_privilege('authenticated',oid,'INSERT,UPDATE,REFERENCES') and not has_table_privilege('anon',oid,'INSERT,UPDATE,DELETE,TRUNCATE,REFERENCES,TRIGGER')) from pg_class where relnamespace='public'::regnamespace and relname in(${tables.map(q).join(',')}))),
 ('private_and_rpc_permissions',${base.functionPrivileges(m.functions.filter(x=>!preserved.includes(x.name)),rpcs)}),
 ('rehearsal_upstream_unchanged',nullif(current_setting('vp.request057_before',true),'') is null or current_setting('vp.request057_before',true)=(select hashes::text from protected))) ${result(true)}`;
 const dry=`BEGIN;\nselect set_config('vp.request057_before',upstream_evidence_hashes::text,true),set_config('vp.request057_preflight',expense_payment_fact_preflight_pass::text,true) from (${pre.trim().replace(/;$/,'')}) p;\nDO $gate$ BEGIN IF current_setting('vp.request057_preflight')<>'true' THEN RAISE EXCEPTION '057 preflight failed; STOP'; END IF; END $gate$;\n-- BEGIN EMBEDDED MIGRATION 057\n${source()}-- END EMBEDDED MIGRATION 057\n${verify}ROLLBACK;\n`;
...
if(require.main===module){for(const [file,sql] of Object.entries(workflow())){if(process.argv.includes('--write'))fs.writeFileSync(file,sql);else assert.equal(fs.readFileSync(file,'utf8'),sql);}console.log('057 artifacts verified');}
