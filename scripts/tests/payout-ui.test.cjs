...
test('Sidebar exact Finance order and exposed Legacy; horizontal nav renders nothing',()=>{
 for(const locale of ['th','en']){const items=financeNavigationItems(buildPermissions({role:'admin'}),locale);assert.deepEqual(items.map(i=>i.group||i.page),['quotations','fee-agreements','billable-charges','invoices','payments','revenue-distribution','payment-documents','expenses','expense-claims','payables','treasury','tax-position','legacy']);assert.deepEqual(items.at(-1).children.map(i=>i.href),['/finance/expense-claims','/finance/compensation','/finance/ledger']);}
