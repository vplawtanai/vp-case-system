...
test('EN changed labels smoke; no changes to calculations, item RPC payloads or Migration 056',()=>{
 ...
 const withoutDeclaration=s=>s.replace('  if (paymentFacts) Object.assign(input, { creator_payment_fact: handling === "personal" ? "personal_paid" : handling });\n','').replace(/^[ \t]*if \(!claim && !companyDeclarationsMatch\(items, (?:fresh|submitted)\)\) \{ setLocalError\("companyDeclarationMismatch"\); return; \}\n/gm,'');
 for(const file of ['app/finance/expenses/forms.tsx','app/finance/expenses/workspace.tsx','app/finance/expenses/request-modal.tsx'])assert.deepEqual(extracts(file,withoutDeclaration(fs.readFileSync(file,'utf8'))),extracts(file,withoutDeclaration(cp.execFileSync('git',['show','HEAD:'+file],{encoding:'utf8'}))),file);
 ...
 const withoutHiddenCompanyPayee=s=>s.replace('companyReview && !payable ? null : ','');
 assert.deepEqual(amounts(withoutHiddenCompanyPayee(fs.readFileSync('app/finance/expenses/forms.tsx','utf8'))),amounts(withoutHiddenCompanyPayee(cp.execFileSync('git',['show','HEAD:app/finance/expenses/forms.tsx'],{encoding:'utf8'}))));
});
