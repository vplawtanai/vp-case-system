...
export const supabase={from(table){
 ...
 if(name==='get_finance_expense_economics'){const i=requests.flatMap(r=>r.items).find(i=>i.id===args.p_expense);return{data:{context:{approved_gross:i?.settlement?.amount||0,original_gross:i?.gross_amount||0,source_vat:0,recoverable_vat:0},decision:null,effective:false,can_manage:true}};}
 if(name==='review_finance_employee_reimbursement'||(name==='review_finance_expense_with_economics'&&args.p_kind==='employee_claim')){
 ...
 if(name==='review_finance_company_purchase_request'||(name==='review_finance_expense_with_economics'&&args.p_kind==='company_purchase')){
  if(operations.has(args.p_operation))return{data:args.p_expense};
  const i=requests.flatMap(r=>r.items).find(i=>i.id===args.p_expense);
...
async function main(){
 ...
 const css=['app/components/ui/vp-ui.module.css','app/components/DetailModal.module.css','app/finance/expenses/expenses.module.css','app/finance/expenses/purchase-request.module.css','app/finance/expenses/item-navigator.module.css','app/finance/expenses/claim-category-combobox.module.css','app/finance/expenses/company.module.css','app/finance/expenses/company-money.module.css','app/finance/payouts/payout.module.css','app/finance/payables/payables.module.css','app/finance/treasury/treasury.module.css','app/finance/statement/statement.module.css'].map(file=>{const prefix=path.basename(file).replaceAll('.','_')+'_';return fs.readFileSync(root+'/'+file,'utf8').replace(/\.([A-Za-z_][A-Za-z_0-9-]*)/g,(_,key)=>'.'+prefix+key).replace(/:global\(([^)]+)\)/g,'$1');}).join('\n');
 ...
 try{
  ...
  if(process.argv.includes('--review-navigator')){
   const scenarios=await require('./review-navigator-browser.cjs')({page,url,out,translate});assert.deepEqual(errors,[]);assert.deepEqual(external,[]);console.log(JSON.stringify({pass:true,scenarios,artifacts:out,externalRequests:0}));return;
  }
