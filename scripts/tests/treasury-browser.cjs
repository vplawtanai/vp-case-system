...
export const supabase={auth:{async getUser(){return{data:{user:{id:'synthetic'}}}},async signOut(){throw Error('Blocked')}},from(name){return{select(){return this},in(){return this},eq(){return this},async single(){return{data:{role:location.search.includes('readonly')?'partner':'admin'}}},then(resolve){if(name==='finance_payouts')throw Error('Direct payout read is revoked');const tables={
 ...
 if(name==='get_finance_expense_requests')return {data:{rows:([0,1].map(i=>({id:'payout-'+i,status:'confirmed',source_model:'expense_v1',confirmed_snapshot_json:{schema_version:2,source_model:'expense_v1',wht:i?0:15,payee:{legal_name:i?'Synthetic claimant':'Big C'},choices:[{expense_id:'expense-'+i,expense:{id:'expense-'+i,origin:i?'employee_claim':'company_purchase',description:'',request_id:i?'22222222-0000-4000-8000-000000000001':'11111111-0000-4000-8000-000000000001',created_by:'requester',client_id:'client',case_id:55}}]}}))).filter(x=>(x.id==='payout-1')===p.p_claims).map(x=>({items:[{payout:{id:x.id,status:x.status},audit:[{event_type:'payment_confirmed',evidence_json:x.confirmed_snapshot_json}]}]})),has_next:false}};
