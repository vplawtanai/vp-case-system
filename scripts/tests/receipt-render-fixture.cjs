...
function render(row) { return renderToStaticMarkup(React.createElement(ReceiptDocument,{receipt:row})); }
function html(row) { return `<!doctype html><html lang="th"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><style>*{box-sizing:border-box}body{margin:0;font-family:Arial,sans-serif}${css.join('\n')}</style><aside data-app-chrome>APPLICATION CHROME</aside><main>${render(row)}</main><button data-app-chrome style="position:fixed;right:0;top:200px">Floating help</button></html>`; }
