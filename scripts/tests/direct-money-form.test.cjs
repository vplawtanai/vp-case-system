...
const { directVatSelection, directVatPatch, directVatChoiceIncomplete, directVatTreatments, reconciliationResult } = require('../../app/finance/direct-money/form-presentation.ts');
const { resolveVatEvidence } = require('../../app/finance/document-decision/shared.ts');
const form = workspaceFixture('app/finance/direct-money/form.tsx', ['DirectMoneyForm', 'DirectAmounts', 'DirectReconciliation']);
function sample() {
...
for (const locale of ['th', 'en']) test(`${locale}: shared form copy, one VAT selector, optional metadata and reconciliation`, () => {
  ...
  assert.doesNotMatch(html, /<dt>(?:Cash|เงินสด)<\/dt>|Actual cash|อยู่ในระบบ VAT|type="checkbox"|before issuing a document|ก่อนออกเอกสาร/);
  ...
  assert.ok(html.includes(translate(locale, 'directMoney.lineReason')));
  assert.ok(html.includes(translate(locale, 'directMoney.vatTreatment')));
