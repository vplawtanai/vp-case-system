...
module.exports=async({page,url,out,translate})=>{
 ...
 await dialog.locator('#claim-review-item').selectOption({index:1});await dialog.getByRole('button',{name:t('companyReject'),exact:true}).click();assert.ok(await dialog.getByRole('alert').count());await dialog.locator('#claim-review-note').fill('Not reimbursable');await dialog.getByRole('button',{name:t('companyReject'),exact:true}).click();await dialog.locator('#claim-approved').waitFor({state:'hidden'});
