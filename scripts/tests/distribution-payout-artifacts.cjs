...
const {candidatePath,contractPath}=require('./distribution-payout-contract.cjs');
...
const files={preflight:'scripts/sql/preflight_distribution_participant_payout_068.sql',dryRun:'scripts/sql/dry_run_distribution_participant_payout_068.sql',verifier:'scripts/sql/verify_distribution_participant_payout_068.sql'};
const pinsPath='scripts/tests/distribution-payout-approved-hashes.json';
...
function preflight(c){return `-- 068 SELECT-only Preflight. Copy this entire file into Supabase SQL Editor.
...
function verifier(c,pins){return `-- 068 SELECT-only Post-Apply Verifier. No DDL/DML or mutable RPC calls.
-- Fail closed until the Production PASS Preflight state/row hashes are pinned.
-- Normalization replaces only the exact verified 068 function delta in memory.
...
FROM checks;
`;}
function dryRun(c,pins){return `-- 068 ROLLBACK-ONLY. Run COMPLETE file; any SQL error is a failed gate.
...
DECLARE actual jsonb; baseline jsonb; failures jsonb; expected jsonb;
 capture_sql text:=${q(prior.captureSql(c))}; candidate_sql text:=$candidate068$${source()}$candidate068$;
BEGIN
 IF ${hash('candidate_sql')}<>${q(c.candidateSha)} THEN RAISE EXCEPTION '068_CANDIDATE_HASH_MISMATCH'; END IF;
 EXECUTE capture_sql INTO baseline;
 IF ${hash('baseline')}<>${q(pins.stateSha)} OR ${hash("baseline->'rows'")}<>${q(pins.rowsSha)} THEN RAISE EXCEPTION '068_APPROVED_PREFLIGHT_REQUIRED_OR_BASELINE_CHANGED'; END IF;
 ...
 IF failures<>'[]'::jsonb OR NOT (${policyCheck}) OR NOT (${prior.v2Check}) THEN RAISE EXCEPTION '068_PRECONDITION_FAILED: %',failures; END IF;
 ...
 ${prior.differences('actual','expected')} INTO failures;
 IF failures<>'[]'::jsonb OR NOT (${policyCheck}) OR NOT (${prior.v2Check}) THEN RAISE EXCEPTION '068_POSTCONDITION_FAILED: %',failures; END IF;
END;
$gate068$;
ROLLBACK;
...
FROM checks;
`;}
function artifacts(c,pins){return {[files.preflight]:preflight(c),[files.dryRun]:dryRun(c,pins),[files.verifier]:verifier(c,pins)};}
