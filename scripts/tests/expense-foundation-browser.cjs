...
const adapter=write('adapter.js',`window.calls=[];export const supabase={auth:{async getUser(){return {data:{user:{id:'synthetic'}}}}},from(name){if(name!=='user_profiles')throw Error('Unexpected table read');return {select(){return this},eq(){return this},async single(){return location.search.includes('finance-only')?{data:{role:'staff'}}:{data:{role:location.search.includes('employee')?'staff':'admin',can_submit_expense_claim:true,can_view_own_expense_claims:true}}}}},async rpc(name){window.calls.push(name);if(name==='get_finance_expense_access')return{data:location.search.includes('finance-only')?${JSON.stringify({...fixture('list').data.access,can_claim:false,can_record:false,can_confirm:false,can_view_accounts:false,is_admin:false})}:location.search.includes('employee')?${JSON.stringify(fixture('employee').data.access)}:${JSON.stringify(fixture('list').data.access)}};throw Error('Forbidden RPC '+name);}};`);
...
async function main(){
 ...
 try{
  ...
  for(const width of [1440,1024,768,390])for(const locale of ['th','en']){
   ...
   for(const flow of ['list','company-review','employee','claim-review','queue','draft','confirmed']){
    ...
    if(flow==='queue'){await page.locator('main button[aria-pressed]').last().click();await page.getByRole('complementary').getByText('3,725.00 THB',{exact:true}).waitFor();await page.getByText('20,245.00 THB',{exact:true}).waitFor();assert.equal(await page.locator('[data-recipient]').count(),3);}
   ...
   console.log('PASS',locale,width,'all five flows + draft/confirmed + navigation keyboard/drawer');
  }
