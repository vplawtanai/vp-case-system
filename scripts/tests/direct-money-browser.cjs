...
async function main(){
 ...
 try{
  ...
  assert.equal(await page.evaluate(()=>window.calls.length),1);const payload=await page.evaluate(()=>window.calls[0]);assert.equal(payload.name,'save_finance_direct_money_receipt');assert.equal(payload.p.p_input.cash_amount,10400);assert.equal(payload.p.p_input.lines[0].wht_rate,3);
