...
fs.writeFileSync(entry,`import React from'react';import{createRoot}from'react-dom/client';import{UiLocaleProvider}from'${root}/lib/i18n/provider.tsx';import{DirectMoneyForm}from'${root}/app/finance/direct-money/form.tsx';import{DirectMoneyDetail}from'${root}/app/finance/direct-money/[id]/page.tsx';import{DirectMoneyList}from'${root}/app/finance/direct-money/list.tsx';import{IncomingMoney}from'${root}/app/finance/payments/page.tsx';const query=new URLSearchParams(location.search),mode=query.get('mode'),initial=${JSON.stringify(input)};if(mode==='custom'){initial.cash_amount=10550;initial.lines[0].wht_base=5000;}createRoot(document.getElementById('root')).render(<UiLocaleProvider initialLocale={query.get('locale')||'en'} pathname="/finance/direct-money/local">{mode==='incoming'?<main><IncomingMoney canManage/></main>:mode==='list'?<DirectMoneyList/>:mode==='detail'||mode==='confirmed'||mode==='partner'?<DirectMoneyDetail id="${id}" canManage={mode!=='partner'}/>:<main><DirectMoneyForm initial={mode==='empty'?undefined:initial}/></main>}</UiLocaleProvider>);`);
async function main(){
 ...
 const css=['app/components/ui/vp-ui.module.css','app/finance/direct-money/direct-money.module.css','app/finance/finance-record-list.module.css','app/finance/payments/incoming-money.module.css','app/finance/payments/money-allocation.module.css','app/finance/payments/vp-distribution.module.css','app/components/DetailModal.module.css'].map(file=>{const prefix=path.basename(file).replaceAll('.','_')+'_';return fs.readFileSync(path.join(root,file),'utf8').replace(/\.([A-Za-z_][A-Za-z_0-9-]*)/g,(_,key)=>'.'+prefix+key);}).join('\n');
 ...
 try{
  ...
  for(const width of [390,768,1024,1440])for(const locale of ['th','en'])for(const mode of ['form','detail','list','incoming']){
   await page.setViewportSize({width,height:1000});await visit(mode,locale);await geometry();
   if(mode==='incoming'){
    assert.equal(await page.getByRole('table').count(),1);assert.equal(await page.locator('[data-status=draft]').count(),1);
    const create=page.getByRole('link',{name:translate(locale,'directMoney.create'),exact:true});await create.focus();
    assert.equal(await create.evaluate(n=>n===document.activeElement),true);
    assert.equal(await create.evaluate(n=>getComputedStyle(n).backgroundColor),'rgb(23, 102, 75)');
  ...
  for(const width of [390,768,1024,1440])for(const locale of ['th','en']){
   ...
   await field('lineActual').fill('10400');await page.locator('[aria-live=polite]').getByText(t('matched'),{exact:true}).waitFor({state:'visible'});assert.equal(await matches(),true);
