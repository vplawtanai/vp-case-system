...
test('22 atomic choices retain requested group order; Other always last, including search and history',()=>{
 assert.equal(claimCategories.length,22);assert.equal(new Set(claimCategories.map(c=>c.value)).size,22);
