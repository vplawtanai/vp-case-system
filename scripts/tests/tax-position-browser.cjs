...
async function main(){
 ...
 try{
  ...
  for(const locale of ['th','en'])for(const width of [390,768,1024,1440]){
   ...
   const technical=page.locator('details').filter({has:page.locator('summary').getByText(t('technical'),{exact:true})}).last();await technical.locator('summary').first().click();assert.equal(await technical.locator('pre:visible').count(),0);await technical.locator('summary').nth(1).click();await technical.locator('pre').waitFor();await technical.locator('summary').first().click();
   ...
   for(const k of ['materialize','periodAction','evidenceAction'])assert.equal(await page.getByRole('button',{name:t(k),exact:true}).count(),0);assert.equal(await page.locator('pre').count(),0);await geometry();
