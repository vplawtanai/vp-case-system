...
const f=fixture(),payee=f.payees[0].id,otherPayee='40000000-0000-4000-8000-000000000099';
f.payees[0].destination={...f.payees[0].destination,bank_name:'Synthetic KTB',account_number:'1234566789'};
f.payees.push({...f.payees[0],id:otherPayee,profile_id:null,kind:'external',legal_name:'Synthetic existing external',tax_id:null,destination:null});
f.components.push({...f.components[0],id:'40000000-0000-4000-8000-000000000098',recipient_id:otherPayee,role_label:'Synthetic other entitlement',gross_amount:777});
f.history.push({id:'40000000-0000-4000-8000-000000000097',payee_id:payee,paid_on:'2026-09-01',status:'cancelled',gross:111,net:111,wht:0});
...
export const supabase={auth:{async getUser(){return{data:{user:{id:'synthetic'}}}},async signOut(){throw Error('Blocked')}},from(name){if(name!=='user_profiles')throw Error('Unexpected table');return{select(){return this},eq(){return this},async single(){return{data:{role:location.search.includes('readonly')?'partner':'admin'}}}}},async rpc(name,p){window.calls.push({name,p});await new Promise(r=>setTimeout(r,30));const d=window.data;
 if(name==='get_finance_payout_workspace'){if(location.search.includes('denied'))return{error:{message:'PAYOUT_PERMISSION_DENIED'}};const result={...structuredClone(d),payout:p.p_payout_id?d.payout:null,components:d.components.filter(e=>e.recipient_id===p.p_payee_id),history:d.history.filter(h=>h.payee_id===p.p_payee_id||(h.id===d.payout?.id&&d.payout?.payee_id===p.p_payee_id))};if(location.search.includes('mismatched'))result.components=d.components;return {data:result};}if(name==='get_finance_payees')return{data:d.payees};
...
async function main(){
 ...
 try{
  ...
  for(const width of [1440,1024,768,390])for(const locale of ['th','en']){
   const t=k=>translate(locale,'payout.'+k);await page.setViewportSize({width,height:1100});await page.goto(url+'/finance/payouts/new');await page.locator('button[lang='+locale+']').first().click();
   ...
   assert.equal(await page.locator('[data-entry-mode=generic]').count(),1);assert.equal(await page.locator('#payout-payee').count(),1);assert.equal(await page.locator('[data-check=checkRecipient]').getAttribute('data-state'),'missing');
   assert.equal(await page.getByText('1,940.00 THB',{exact:true}).count(),0);assert.equal(await page.getByText('40000000-0000-4000-8000-000000000097',{exact:true}).count(),0);
   await geometry();await page.screenshot({path:out+`/generic-${locale}-${width}.png`,fullPage:true});
   ...
   assert.equal(await page.locator('#payout-payee').count(),0);await page.getByRole('button',{name:t('changeRecipient'),exact:true}).click();await page.locator('[data-fixture-payables]').waitFor();assert.equal(await page.getByRole('dialog').count(),0);
   await page.getByRole('link',{name:'Synthetic generic entry',exact:true}).click();await page.locator('button[lang='+locale+']').first().click();await page.getByLabel(t('recipient'),{exact:true}).selectOption(payee);await page.getByRole('button',{name:t('selectAll'),exact:true}).waitFor();
   assert.equal(await page.locator('#payout-payee').count(),0);assert.equal(await page.locator('[data-entry-mode=contextual]').count(),1);assert.equal(await page.getByText('777.00 THB',{exact:true}).count(),0);
   ...
   await page.getByRole('button',{name:t('selectAll'),exact:true}).click();const initialRates=page.getByLabel(t('rate'),{exact:true});await initialRates.nth(0).selectOption('0');await initialRates.nth(1).selectOption('3');
   await page.getByLabel(t('account'),{exact:true}).selectOption('bank:'+f.accounts[0].account_id);await page.getByLabel(t('date'),{exact:true}).fill('2026-09-18');
   await page.getByText('3,069.08 THB',{exact:true}).first().waitFor();await geometry();await page.evaluate(()=>window.scrollTo(0,0));await page.screenshot({path:out+`/payout-${locale}-${width}.png`,fullPage:true});
   ...
   assert.ok(await modal.getByText(f.payees[0].legal_name,{exact:true}).isVisible());assert.ok(await modal.getByText('Synthetic KTB ••••6789',{exact:true}).isVisible());
   const treasury=modal.locator('[data-confirmation-balance]');for(const value of ['29,560.00 THB','3,069.08 THB','26,490.92 THB'])assert.ok((await treasury.innerText()).includes(value));
   const effects=modal.locator('[data-confirmation-effects]');for(const value of ['3,104.00 THB','3,069.08 THB','34.92 THB','KBANK','Draft','Confirmed'])assert.ok((await effects.innerText()).includes(value),value);
   assert.equal(await effects.locator('li').count(),5);assert.ok((await modal.innerText()).includes(t('confirmHelp')));
   await geometry();await page.screenshot({path:out+`/confirmation-top-${locale}-${width}.png`});
   const finalConfirm=modal.getByRole('button',{name:t('confirm'),exact:true}),reviewAck=modal.getByLabel(t('ackReviewed'),{exact:true});
   const paidLabel=translate(locale,'payout.paidBankAck',{amount:'3,069.08 THB',destination:'••••6789'}),paidAck=modal.getByLabel(paidLabel,{exact:true});
   assert.equal(await modal.getByRole('checkbox').count(),2);assert.ok(await finalConfirm.isDisabled());assert.equal(await finalConfirm.getAttribute('aria-describedby'),'payout-confirm-ack-required');
   await reviewAck.focus();await page.keyboard.press('Space');assert.ok(await reviewAck.isChecked());assert.ok(await finalConfirm.isDisabled());
   await page.keyboard.press('Tab');assert.equal(await paidAck.evaluate(e=>e===document.activeElement),true);await page.keyboard.press('Space');assert.ok(await paidAck.isChecked());assert.ok(await finalConfirm.isEnabled());
   await reviewAck.uncheck();assert.ok(await finalConfirm.isDisabled());assert.equal(await page.evaluate(()=>window.calls.filter(c=>c.name==='confirm_finance_payout').length),0);
   await reviewAck.check();assert.ok(await finalConfirm.isEnabled());assert.equal(await finalConfirm.getAttribute('aria-describedby'),null);
   for(const checkbox of [reviewAck,paidAck])assert.ok(await checkbox.evaluate(e=>{const box=e.getBoundingClientRect(),label=e.nextElementSibling.getBoundingClientRect();return box.right<=label.left&&Math.abs(box.top-label.top)<8;}),'checkbox remains beside first label line');
   await finalConfirm.scrollIntoViewIfNeeded();await geometry();await page.screenshot({path:out+`/confirmation-bottom-${locale}-${width}.png`});
   await finalConfirm.focus();await page.keyboard.press('Tab');assert.equal(await modal.getByRole('button',{name:translate(locale,'common.actions.closeDetails'),exact:true}).evaluate(e=>e===document.activeElement),true);
   ...
   await review.click();await modal.waitFor();assert.equal(await reviewAck.isChecked(),false);assert.equal(await paidAck.isChecked(),false);assert.ok(await finalConfirm.isDisabled());
   await reviewAck.check();await paidAck.check();await finalConfirm.click();await modal.waitFor({state:'hidden'});
   ...
   await page.goto(url+'/finance/payouts/new?missing=1&payee='+payee);await page.locator('button[lang='+locale+']').first().click();
   ...
   const change=page.getByRole('button',{name:t('changeRecipient'),exact:true});await change.focus();await page.keyboard.press('Enter');const reset=page.getByRole('dialog');await reset.waitFor();assert.ok(await reset.getByText(t('changeRecipientWarning'),{exact:true}).isVisible());await page.keyboard.press('Tab');assert.equal(await reset.locator(':focus').count(),1);await geometry();await page.screenshot({path:out+`/change-${locale}-${width}.png`});await reset.getByRole('button',{name:translate(locale,'common.actions.cancel'),exact:true}).click();assert.equal(await change.evaluate(e=>e===document.activeElement),true);assert.equal(await selects.nth(0).inputValue(),'3');
   const fix=page.getByRole('button',{name:t('fixPayee'),exact:true}).first();await fix.focus();await page.keyboard.press('Enter');const repair=page.getByRole('dialog');await repair.waitFor();await page.keyboard.press('Tab');assert.equal(await repair.locator(':focus').count(),1);
   ...
   await page.goto(url+'/finance/payouts/new?missing=1&payee='+payee);await page.locator('button[lang='+locale+']').first().click();await page.getByRole('button',{name:t('selectAll'),exact:true}).click();
   ...
   assert.equal(await page.locator('[data-check=requiredTax]').getAttribute('data-state'),'ready');await geometry();
   // Discarding unsaved Pam choices returns to Payables, then generic selection starts clean for another UUID.
   await page.getByLabel('5. '+t('note'),{exact:true}).fill('Do not carry to the next payee');await page.getByLabel(t('date'),{exact:true}).fill('2026-09-01');
   await page.getByRole('button',{name:t('changeRecipient'),exact:true}).click();const leave=page.getByRole('dialog');await leave.waitFor();await leave.getByRole('button',{name:t('backChooseRecipient'),exact:true}).click();await page.locator('[data-fixture-payables]').waitFor();
   assert.equal(await page.evaluate(()=>window.calls.some(c=>!c.name.startsWith('get_'))),false);
   await page.getByRole('link',{name:'Synthetic generic entry',exact:true}).click();await page.locator('button[lang='+locale+']').first().click();await page.getByLabel(t('recipient'),{exact:true}).selectOption(otherPayee);
   await page.getByText('777.00 THB',{exact:true}).waitFor();assert.equal(await page.getByLabel(t('account'),{exact:true}).inputValue(),'');assert.equal(await page.getByLabel('5. '+t('note'),{exact:true}).inputValue(),'');assert.notEqual(await page.getByLabel(t('date'),{exact:true}).inputValue(),'2026-09-01');assert.equal(await page.getByLabel(t('rate'),{exact:true}).count(),0);assert.equal(await page.locator('input[type=checkbox]:checked').count(),0);assert.equal(await page.getByText('1,940.00 THB',{exact:true}).count(),0);await page.getByText(t('emptyHistory'),{exact:true}).waitFor();
   assert.equal(await page.locator('[data-check=checkRecipient]').getAttribute('data-state'),'ready');assert.equal(await page.getByRole('list',{name:t('progress')}).locator('[data-state=complete]').count(),0);
   console.log(`PASS ${locale} ${width}: lifecycle, mixed WHT, blockers, repair preservation, cash/no-WHT, keyboard, overflow`);
  }
  for(const query of ['payee=not-a-uuid','payee=','payee=40000000-0000-4000-8000-000000000088','payee='+payee+'&denied=1','payee='+payee+'&mismatched=1']){
   await page.goto(url+'/finance/payouts/new?'+query);await page.getByRole('alert').waitFor();assert.equal(await page.locator('[data-entry-mode]').count(),0);assert.equal(await page.getByText(f.payees[0].legal_name,{exact:true}).count(),0);assert.equal(await page.locator('#payout-payee').count(),0);assert.equal(await page.evaluate(()=>window.calls.some(c=>!c.name.startsWith('get_'))),false);
   if(query==='payee=not-a-uuid'||query==='payee=')assert.equal(await page.evaluate(()=>window.calls.length),0);
  }
  await page.goto(url+'/finance/payouts/new?payee='+payee+'&readonly=1');await page.locator('[data-entry-mode=contextual]').waitFor();assert.equal(await page.locator('#payout-payee').count(),0);assert.equal(await page.getByRole('button',{name:translate('th','payout.changeRecipient'),exact:true}).count(),0);assert.equal(await page.getByRole('button',{name:translate('th','payout.fixPayee'),exact:true}).count(),0);
