...
export const supabase={auth:{async getUser(){return{data:{user:{id:'synthetic'}}}},async signOut(){throw Error('Blocked')}},from(name){if(name!=='user_profiles')throw Error('Unexpected table');return{select(){return this},eq(){return this},async single(){return{data:{role:location.search.includes('readonly')?'partner':'admin'}}}}},async rpc(name,p){window.calls.push({name,p});await new Promise(r=>setTimeout(r,30));const d=window.data;
 ...
 if(name==='save_finance_payee'){if(p.p_profile_id!==null||!p.p_input.legal_name)throw Error('Unexpected synthetic Payee');d.payees.push({id:p.p_id,kind:'external',profile_id:null,legal_name:p.p_input.legal_name,entity_type:p.p_input.entity_type,tax_id:p.p_input.tax_id,is_active:true,version:1,destination:p.p_input.destination});return{data:p.p_id};}
...
async function main(){
 ...
 try{
  ...
  const geometry=async()=>assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth>innerWidth+1),false,'page overflow');
  for(const width of [1440,1024,768,390])for(const locale of ['th','en']){
   ...
   await page.getByText(t('noRights'),{exact:true}).waitFor();await page.getByText(t('bankMissing'),{exact:true}).waitFor();
   ...
   await page.getByText('3,010.88 THB',{exact:true}).waitFor();await geometry();await page.evaluate(()=>window.scrollTo(0,0));await page.screenshot({path:out+`/payout-${locale}-${width}.png`,fullPage:true});
