...
const f=fixture();
const render=(locale,data=f,states={})=>view.render(locale,states,{data,offset:0,loading:false,busy:false,onPage:()=>{},onOpening:()=>{},onMaterialize:()=>{}},'TreasuryDashboard');
test('Known system total and pending cash use authoritative values; unknown is not zero; source data untouched',()=>{
 const before=JSON.stringify(f),o=treasuryOverview(f);assert.deepEqual(o.balances,[{currency:'THB',amount:49560}]);assert.deepEqual(o.pending,{count:5,totals:[{currency:'THB',amount:31409.81}]});
...
test('Presentation totals separate currencies, keep negative known balances, and never add WHT to cash',()=>{
 const d={...f,accounts:[...f.accounts,{...f.accounts[0],currency:'USD',system_balance:-10.01}],pending_sources:[...f.pending_sources,{...f.pending_sources[0],currency:'USD',cash_amount:0.1,wht_amount:999}]};
 assert.deepEqual(treasuryOverview(d).balances,[{currency:'THB',amount:49560},{currency:'USD',amount:-10.01}]);assert.deepEqual(treasuryOverview(d).pending.totals,[{currency:'THB',amount:31409.81},{currency:'USD',amount:0.1}]);
});
...
for(const locale of ['th','en']){
 ...
 test(`${locale}: owner summary, compact account states, queue/table hierarchy and collapsed maintenance`,()=>{
  const html=render(locale);for(const text of ['49,560.00','31,409.81','29,560.00 THB','20,000.00 THB','10,400.00 THB','19,160.00 THB',t('knownTotal'),t('pendingTotal'),t('unknownAccounts')])assert.ok(html.includes(text),text);
  assert.equal((html.match(/data-balance="unknown"/g)||[]).length,2);assert.equal((html.match(/data-balance="known"/g)||[]).length,2);
  assert.equal((html.match(/<table\b/g)||[]).length,2);assert.ok(html.indexOf('treasury-accounts')<html.indexOf('treasury-pending'));assert.ok(html.indexOf('treasury-pending')<html.indexOf('treasury-movements'));
  assert.equal((html.match(/<details/g)||[]).length,3);assert.doesNotMatch(html,/<details[^>]*\sopen/);assert.doesNotMatch(html,/<pre/);
...
test('Mutation RPC contracts, modal implementation and navigation wiring remain unchanged',()=>{
 ...
 for(const file of ['app/components/AppTopNav.tsx','app/finance/finance-sidebar.module.css','app/finance/FinanceSubNav.tsx','app/finance/finance-navigation.ts'])assert.equal(fs.readFileSync(file,'utf8'),cp.execFileSync('git',['show','309b867:'+file],{encoding:'utf8'}));
});
