...
async function main() {
  ...
  const css = ['app/components/ui/vp-ui.module.css', 'app/components/DetailModal.module.css', 'app/components/LanguageSelector.module.css', 'app/clients/client-form.module.css', 'app/clients/tax-identity.module.css'].map(file => fs.readFileSync(path.join(root, file), 'utf8').replace(/\.([a-zA-Z_][\w-]*)/g, '.' + path.basename(file, '.module.css') + '__$1')).join('\n');
