...
test('Pending cash copy distinguishes confirmed money from distribution without adding a distribution dependency',()=>{
 const expected={
  th:{pending:'เงินรับที่รอบันทึกเข้าความเคลื่อนไหวเงินจริง',pendingHelp:'รายการที่ยืนยันว่ารับเงินจริงแล้ว แต่ยังไม่ได้บันทึกเข้า Cashbook',materialize:'บันทึกเงินเข้า'},
  en:{pending:'Receipts awaiting cashbook entry',pendingHelp:'Confirmed real-money receipts not yet recorded in the Cashbook.',materialize:'Record cash inflow'},
 };
 ...
 const ui=fs.readFileSync('app/finance/treasury/page.tsx','utf8')+fs.readFileSync('app/finance/treasury/dashboard-view.tsx','utf8');
