...
-- END EMBEDDED MIGRATION 033

do $gross_first_dry_run_tests$
declare
  v_result jsonb;
  v_gross jsonb;
  v_rows jsonb;
...
begin
  select public.finance_compute_quotation_percentage_allocation_gross_first(
    ...
    '[{"installment_id":"i1","installment_no":1,"percentage":50,"items":[{"allocation_id":"a1","item_id":"item-1","allocation_percentage":50}]},{"installment_id":"i2","installment_no":2,"percentage":25,"items":[{"allocation_id":"a2","item_id":"item-1","allocation_percentage":25}]},{"installment_id":"i3","installment_no":3,"percentage":25,"items":[{"allocation_id":"a3","item_id":"item-1","allocation_percentage":25}]}]'::jsonb,
    'proportional_all_items'
  ) into v_result;
  select jsonb_agg((row.value->>'total_amount')::numeric order by row.ordinality),
    sum((row.value->>'amount_before_tax')::numeric), sum((row.value->>'vat_amount')::numeric)
  into v_gross, v_before, v_vat
  from jsonb_array_elements(v_result->'installments') with ordinality as row(value, ordinality);
  if v_gross <> '[10000,5000,5000]'::jsonb or v_before <> 18691.59 or v_vat <> 1308.41 then
    raise exception 'Gross-first test 1 failed: %', v_result;
  end if;

  select public.finance_compute_quotation_percentage_allocation_gross_first(
    '[{"item_id":"item-1","amount_before_tax":20000,"vat_amount":0,"total_amount":20000,"sort_order":0}]'::jsonb,
    '[{"installment_id":"i1","installment_no":1,"percentage":50,"items":[{"allocation_id":"a1","item_id":"item-1","allocation_percentage":50}]},{"installment_id":"i2","installment_no":2,"percentage":25,"items":[{"allocation_id":"a2","item_id":"item-1","allocation_percentage":25}]},{"installment_id":"i3","installment_no":3,"percentage":25,"items":[{"allocation_id":"a3","item_id":"item-1","allocation_percentage":25}]}]'::jsonb,
    'proportional_all_items'
  ) into v_result;
  select jsonb_agg((row.value->>'total_amount')::numeric order by row.ordinality), sum((row.value->>'vat_amount')::numeric)
  into v_gross, v_vat from jsonb_array_elements(v_result->'installments') with ordinality as row(value, ordinality);
  if v_gross <> '[10000,5000,5000]'::jsonb or v_vat <> 0 then raise exception 'Gross-first test 2 failed: %', v_result; end if;

  select public.finance_compute_quotation_percentage_allocation_gross_first(
    '[{"item_id":"item-1","amount_before_tax":100,"vat_amount":0,"total_amount":100,"sort_order":0}]'::jsonb,
    '[{"installment_id":"i1","installment_no":1,"percentage":33.333333,"items":[{"allocation_id":"a1","item_id":"item-1","allocation_percentage":33.333333}]},{"installment_id":"i2","installment_no":2,"percentage":33.333333,"items":[{"allocation_id":"a2","item_id":"item-1","allocation_percentage":33.333333}]},{"installment_id":"i3","installment_no":3,"percentage":33.333334,"items":[{"allocation_id":"a3","item_id":"item-1","allocation_percentage":33.333334}]}]'::jsonb,
    'proportional_all_items'
  ) into v_result;
  select jsonb_agg((row.value->>'total_amount')::numeric order by row.ordinality)
  into v_gross from jsonb_array_elements(v_result->'installments') with ordinality as row(value, ordinality);
  if v_gross <> '[33.33,33.33,33.34]'::jsonb then raise exception 'Gross-first test 3 failed: %', v_result; end if;

  select public.finance_compute_quotation_percentage_allocation_gross_first(
    '[{"item_id":"item-1","amount_before_tax":9345.79,"vat_amount":654.21,"total_amount":10000,"sort_order":0},{"item_id":"item-2","amount_before_tax":9345.80,"vat_amount":654.20,"total_amount":10000,"sort_order":1}]'::jsonb,
    '[{"installment_id":"i1","installment_no":1,"percentage":50,"items":[{"allocation_id":"a1","item_id":"item-1","allocation_percentage":50},{"allocation_id":"a2","item_id":"item-2","allocation_percentage":50}]},{"installment_id":"i2","installment_no":2,"percentage":25,"items":[{"allocation_id":"a3","item_id":"item-1","allocation_percentage":25},{"allocation_id":"a4","item_id":"item-2","allocation_percentage":25}]},{"installment_id":"i3","installment_no":3,"percentage":25,"items":[{"allocation_id":"a5","item_id":"item-1","allocation_percentage":25},{"allocation_id":"a6","item_id":"item-2","allocation_percentage":25}]}]'::jsonb,
    'proportional_all_items'
  ) into v_result;
  select sum((item.value->>'total_amount')::numeric), sum((item.value->>'amount_before_tax')::numeric), sum((item.value->>'vat_amount')::numeric)
  into v_total, v_before, v_vat
  from jsonb_array_elements(v_result->'installments') as installment(value)
  cross join lateral jsonb_array_elements(installment.value->'items') as item(value);
  select jsonb_agg((row.value->>'total_amount')::numeric order by row.ordinality)
  into v_gross from jsonb_array_elements(v_result->'installments') with ordinality as row(value, ordinality);
  select jsonb_object_agg(item_id, jsonb_build_array(amount_before_tax, vat_amount, total_amount))
  into v_rows
  from (
    select
      item.value->>'item_id' as item_id,
      sum((item.value->>'amount_before_tax')::numeric) as amount_before_tax,
      sum((item.value->>'vat_amount')::numeric) as vat_amount,
      sum((item.value->>'total_amount')::numeric) as total_amount
    from jsonb_array_elements(v_result->'installments') as installment(value)
    cross join lateral jsonb_array_elements(installment.value->'items') as item(value)
    group by item.value->>'item_id'
  ) as item_totals;
  if v_gross <> '[10000,5000,5000]'::jsonb
    or v_total <> 20000
    or v_before <> 18691.59
    or v_vat <> 1308.41
    or v_rows <> '{"item-1":[9345.79,654.21,10000],"item-2":[9345.80,654.20,10000]}'::jsonb
  then
    raise exception 'Gross-first test 4 failed: %', v_result;
  end if;
  ...
  select public.finance_compute_quotation_percentage_allocation_gross_first(
    ...
    '[{"installment_id":"i1","installment_no":1,"percentage":50,"items":[{"allocation_id":"a1","item_id":"taxable","allocation_percentage":50},{"allocation_id":"a2","item_id":"non-vat","allocation_percentage":50}]},{"installment_id":"i2","installment_no":2,"percentage":25,"items":[{"allocation_id":"a3","item_id":"taxable","allocation_percentage":25},{"allocation_id":"a4","item_id":"non-vat","allocation_percentage":25}]},{"installment_id":"i3","installment_no":3,"percentage":25,"items":[{"allocation_id":"a5","item_id":"taxable","allocation_percentage":25},{"allocation_id":"a6","item_id":"non-vat","allocation_percentage":25}]}]'::jsonb,
    'proportional_all_items'
  ) into v_result;
  select sum((row.value->>'amount_before_tax')::numeric), sum((row.value->>'vat_amount')::numeric)
  into v_before, v_vat from jsonb_array_elements(v_result->'installments') as row(value);
  select jsonb_agg((row.value->>'total_amount')::numeric order by row.ordinality)
  into v_gross from jsonb_array_elements(v_result->'installments') with ordinality as row(value, ordinality);
  if v_gross <> '[10000,5000,5000]'::jsonb or v_before <> 19345.79 or v_vat <> 654.21 then raise exception 'Gross-first test 5 failed: %', v_result; end if;

  select public.finance_compute_quotation_percentage_allocation_gross_first(
    '[{"item_id":"exclusive","amount_before_tax":10000,"vat_amount":700,"total_amount":10700,"sort_order":0},{"item_id":"non-vat","amount_before_tax":9300,"vat_amount":0,"total_amount":9300,"sort_order":1}]'::jsonb,
    '[{"installment_id":"i1","installment_no":1,"percentage":50,"items":[{"allocation_id":"a1","item_id":"exclusive","allocation_percentage":50},{"allocation_id":"a2","item_id":"non-vat","allocation_percentage":50}]},{"installment_id":"i2","installment_no":2,"percentage":25,"items":[{"allocation_id":"a3","item_id":"exclusive","allocation_percentage":25},{"allocation_id":"a4","item_id":"non-vat","allocation_percentage":25}]},{"installment_id":"i3","installment_no":3,"percentage":25,"items":[{"allocation_id":"a5","item_id":"exclusive","allocation_percentage":25},{"allocation_id":"a6","item_id":"non-vat","allocation_percentage":25}]}]'::jsonb,
    'proportional_all_items'
  ) into v_result;
  select sum((row.value->>'amount_before_tax')::numeric), sum((row.value->>'vat_amount')::numeric)
  into v_before, v_vat from jsonb_array_elements(v_result->'installments') as row(value);
  select jsonb_agg((row.value->>'total_amount')::numeric order by row.ordinality)
  into v_gross from jsonb_array_elements(v_result->'installments') with ordinality as row(value, ordinality);
  if v_gross <> '[10000,5000,5000]'::jsonb or v_before <> 19300 or v_vat <> 700 then raise exception 'Gross-first test 6 failed: %', v_result; end if;

  select public.finance_compute_quotation_percentage_allocation_gross_first(
    '[{"item_id":"item-1","amount_before_tax":93.46,"vat_amount":6.54,"total_amount":100,"sort_order":0}]'::jsonb,
    '[{"installment_id":"i1","installment_no":1,"percentage":14.285714,"items":[{"allocation_id":"a1","item_id":"item-1","allocation_percentage":14.285714}]},{"installment_id":"i2","installment_no":2,"percentage":14.285714,"items":[{"allocation_id":"a2","item_id":"item-1","allocation_percentage":14.285714}]},{"installment_id":"i3","installment_no":3,"percentage":14.285714,"items":[{"allocation_id":"a3","item_id":"item-1","allocation_percentage":14.285714}]},{"installment_id":"i4","installment_no":4,"percentage":14.285714,"items":[{"allocation_id":"a4","item_id":"item-1","allocation_percentage":14.285714}]},{"installment_id":"i5","installment_no":5,"percentage":14.285714,"items":[{"allocation_id":"a5","item_id":"item-1","allocation_percentage":14.285714}]},{"installment_id":"i6","installment_no":6,"percentage":14.285714,"items":[{"allocation_id":"a6","item_id":"item-1","allocation_percentage":14.285714}]},{"installment_id":"i7","installment_no":7,"percentage":14.285716,"items":[{"allocation_id":"a7","item_id":"item-1","allocation_percentage":14.285716}]}]'::jsonb,
    'proportional_all_items'
  ) into v_result;
  select sum((row.value->>'total_amount')::numeric), sum((row.value->>'amount_before_tax')::numeric), sum((row.value->>'vat_amount')::numeric)
  into v_total, v_before, v_vat from jsonb_array_elements(v_result->'installments') as row(value);
  if v_total <> 100 or v_before <> 93.46 or v_vat <> 6.54 then raise exception 'Gross-first test 7 failed: %', v_result; end if;

  select public.finance_compute_quotation_percentage_allocation_gross_first(
    '[{"item_id":"taxable","amount_before_tax":18691.59,"vat_amount":1308.41,"total_amount":20000,"sort_order":0},{"item_id":"non-vat","amount_before_tax":10000,"vat_amount":0,"total_amount":10000,"sort_order":1}]'::jsonb,
    '[{"installment_id":"i1","installment_no":1,"percentage":100,"items":[{"allocation_id":"a1","item_id":"taxable","allocation_percentage":50},{"allocation_id":"a2","item_id":"non-vat","allocation_percentage":100}]},{"installment_id":"i2","installment_no":2,"percentage":100,"items":[{"allocation_id":"a3","item_id":"taxable","allocation_percentage":25}]},{"installment_id":"i3","installment_no":3,"percentage":100,"items":[{"allocation_id":"a4","item_id":"taxable","allocation_percentage":25}]}]'::jsonb,
    'per_item'
  ) into v_result;
  select jsonb_agg((row.value->>'total_amount')::numeric order by row.ordinality)
  into v_gross from jsonb_array_elements(v_result->'installments') with ordinality as row(value, ordinality);
  select jsonb_object_agg(item_id, total_amount)
  into v_rows
  from (
    select item.value->>'item_id' as item_id, sum((item.value->>'total_amount')::numeric) as total_amount
    from jsonb_array_elements(v_result->'installments') as installment(value)
    cross join lateral jsonb_array_elements(installment.value->'items') as item(value)
    group by item.value->>'item_id'
  ) as item_totals;
  if v_gross <> '[20000,5000,5000]'::jsonb
    or v_rows <> '{"non-vat":10000,"taxable":20000}'::jsonb
  then
    raise exception 'Gross-first per-item test failed: %', v_result;
  end if;

  if (select jsonb_agg(installment.total_amount order by installment.installment_no)
      from public.finance_billing_installments as installment
      where installment.billing_plan_id = 'aa7b83b1-f702-4eca-a8ec-5b07e411b049'::uuid)
      <> '[10000.01,5000.00,4999.99]'::jsonb
    or exists (select 1 from public.finance_invoices where billing_plan_id = 'aa7b83b1-f702-4eca-a8ec-5b07e411b049'::uuid)
    or exists (select 1 from public.finance_billing_installment_charge_bridges where billing_plan_id = 'aa7b83b1-f702-4eca-a8ec-5b07e411b049'::uuid)
    or exists (
      select 1
      ...
      where invoice.billing_plan_id = 'aa7b83b1-f702-4eca-a8ec-5b07e411b049'::uuid
    )
    or exists (
      select 1
  ...
  then
    raise exception 'Historical UAT lineage changed during dry-run';
  end if;
end;
$gross_first_dry_run_tests$;

select
  'gross_first_contract_tests_passed'::text as dry_run_result,
  (select count(*) from public.finance_invoices where billing_plan_id = 'aa7b83b1-f702-4eca-a8ec-5b07e411b049'::uuid) as affected_invoice_count,
  (select count(*) from public.finance_billing_installment_charge_bridges where billing_plan_id = 'aa7b83b1-f702-4eca-a8ec-5b07e411b049'::uuid) as affected_bridge_count,
  (select count(distinct allocation.payment_id)
    from public.finance_payment_invoice_allocations as allocation
    join public.finance_invoices as invoice on invoice.id = allocation.invoice_id
    where invoice.billing_plan_id = 'aa7b83b1-f702-4eca-a8ec-5b07e411b049'::uuid) as affected_payment_count,
  (select count(*)
    from public.finance_cash_transactions as cash_transaction
    join public.finance_payment_invoice_allocations as allocation on allocation.payment_id = cash_transaction.source_payment_id
    join public.finance_invoices as invoice on invoice.id = allocation.invoice_id
    where invoice.billing_plan_id = 'aa7b83b1-f702-4eca-a8ec-5b07e411b049'::uuid) as affected_cash_transaction_count;
