-- Combined VP-RTI pre-issue technical preflight: ONE SELECT-only statement, ONE row, NO RPCs.
...
with
k as (select false as after_issue,
...
select to_jsonb(checks) as checks,
  ...
  not exists(select 1 from jsonb_each(to_jsonb(checks)) e where e.value is distinct from 'true'::jsonb)
    as combined_vp_rti_preissue_pass
