-- Transactional Production dry-run for Migration 024.
-- The Void RPC is not invoked and both UAT Invoices remain untouched.

begin;

-- BEGIN EXACT MIGRATION 024
...
-- END EXACT MIGRATION 024

do $invoice_void_dry_run_assertions$
declare
  v_void_rpc_definition text;
begin
  if to_regprocedure('public.assert_finance_invoice_has_no_void_dependencies(uuid)') is null
    or to_regprocedure('public.void_finance_invoice(uuid,text,boolean)') is null
  then
    raise exception 'Migration 024 functions did not compile inside the transaction';
  end if;

  v_void_rpc_definition := pg_get_functiondef(
    'public.void_finance_invoice(uuid,text,boolean)'::regprocedure
  );

  if position('invoice_no is distinct from v_invoice.invoice_no' in lower(v_void_rpc_definition)) = 0
    or position('issued_snapshot_json is distinct from v_invoice.issued_snapshot_json' in lower(v_void_rpc_definition)) = 0
    or position('payment_destination_snapshot_json is distinct from v_invoice.payment_destination_snapshot_json' in lower(v_void_rpc_definition)) = 0
  then
    raise exception 'Migration 024 issued Invoice evidence-preservation contract is incomplete';
  end if;

  if position('status = ''ready_to_invoice''' in v_void_rpc_definition) = 0
    or position('invoiced_at = null' in lower(v_void_rpc_definition)) = 0
    or position('v_plan.status = ''completed''' in v_void_rpc_definition) = 0
    or position('status = ''active''' in v_void_rpc_definition) = 0
  then
    raise exception 'Migration 024 Installment or Billing Plan reopen contract is incomplete';
  end if;
  ...
  if not exists (
    ...
    raise exception 'Migration 024 installment audit extension is missing inside the transaction';
  end if;
  ...
  if exists (
    with scenarios as (
      select *
      from (values
        ('no_payment', null::text, 0::numeric, true),
        ('cancelled_payment', 'cancelled', 0::numeric, true),
        ('reversed_payment', 'reversed', 0::numeric, true),
        ('active_draft', 'draft', 0::numeric, false),
        ('confirmed_partial', 'confirmed', 10000::numeric, false),
        ('confirmed_full', 'confirmed', 15000::numeric, false)
      ) as scenario(name, payment_status, confirmed_settlement, expected_allowed)
    )
    select 1
    from scenarios
    where (
      coalesce(payment_status not in ('draft', 'confirmed'), true)
      and confirmed_settlement = 0
    ) is distinct from expected_allowed
  ) then
    raise exception 'Migration 024 Payment blocker scenario contract is inconsistent';
  end if;

  if (select count(*) from public.finance_invoices where invoice_no = 'VP-IV-202608-000001' and document_status = 'issued') <> 1
    or (select count(*) from public.finance_invoices where invoice_no = 'VP-IV-202608-000002' and document_status = 'issued') <> 1
    or exists (select 1 from public.finance_invoices where document_status = 'voided')
  then
    raise exception 'Migration 024 dry-run must not change either UAT Invoice';
  end if;
  ...
  if not exists (
    ...
    raise exception 'Replacement Invoice uniqueness contract is not preserved';
  end if;
end;
$invoice_void_dry_run_assertions$;

select
  'MIGRATION_024_TRANSACTIONAL_DRY_RUN' as report_section,
  to_regprocedure('public.assert_finance_invoice_has_no_void_dependencies(uuid)') is not null
    as dependency_guard_compiled,
  to_regprocedure('public.void_finance_invoice(uuid,text,boolean)') is not null
    as void_rpc_compiled,
  exists (
    select 1 from pg_constraint
    where conrelid = 'public.finance_billing_installment_audit_events'::regclass
      and conname = 'finance_billing_installment_audit_events_type_check'
      and lower(pg_get_constraintdef(oid)) like '%invoice_voided_reopened%'
  ) as installment_audit_extension_present,
  (select count(*) = 1 from public.finance_invoices where invoice_no = 'VP-IV-202608-000001' and document_status = 'issued')
    as invoice_000001_untouched,
  (select count(*) = 1 from public.finance_invoices where invoice_no = 'VP-IV-202608-000002' and document_status = 'issued')
    as invoice_000002_untouched,
  (select count(*) from public.finance_payments) as payment_rows_unchanged,
  (select count(*) from public.finance_company_ledger) as ledger_rows_unchanged,
  (select count(*) from public.finance_compensation_batches) as compensation_rows_unchanged,
  true as rollback_required;
