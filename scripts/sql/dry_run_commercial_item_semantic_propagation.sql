...
do $dry_run_assertions$
begin
  if to_regprocedure('public.inherit_finance_fee_agreement_item_semantics()') is null
    or to_regprocedure('public.inherit_finance_billing_installment_item_semantics()') is null
  then raise exception 'Migration 035 dry-run did not install semantic propagation functions'; end if;
  if not exists (select 1 from pg_trigger where tgname='finance_fee_agreement_item_semantics_before_write' and not tgisinternal)
    or not exists (select 1 from pg_trigger where tgname='finance_billing_installment_item_semantics_before_write' and not tgisinternal)
  then raise exception 'Migration 035 dry-run did not install semantic propagation triggers'; end if;
end;
$dry_run_assertions$;
